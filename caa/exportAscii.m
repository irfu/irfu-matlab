function exportAscii(var,varargin)
%exportAscii export variable into ascii/CEF file with header
% exportAscii(var,[option,value])
%
% Options:
...
%	exportAscii(Etemp,'var_name','diE3p1234')
%	exportAscii(Etemp,'var_name','diE3p1234',...
...
while have_options
	...
	if length(args)>1
		...
		case 'var_name'
			...
			else, c_log('fcal','wrong ArgType : var_name must be string')
			end
		case 'comm'
			if ischar(args{2}), comment = args{2};
			else, c_log('fcal','wrong ArgType : comm must be string')
			end
		case 'f_name'
			if ischar(args{2}), file_name = args{2};
			else, c_log('fcal','wrong ArgType : f_name must be string')
			end
		case 'mode'
			if ischar(args{2}), 
				switch(args{2})
				...
				otherwise
					...
					c_log('fcal','Using default mode : ''plain''');
				end
			else, c_log('fcal',...
				'wrong ArgType : mode must be one of : ''plain'', ''cef'' or ''caa''')
			end
		...
		error('caa:wrongArgType','use exportAscii(..,''option'',''value'')')
	end
end
...
if CEF
	...
	c_log('save',[var_name{1} ' resolution: ' resolution ' Delta: ' delta])
	mask = '';
...
if isempty(file_name)
	if CAA % we have special names for CAA
		file_name = ...
		['C' num2str(cl_id) '_CAA_EFW_' var_name{1} '_' makeFName(var(1,1)) '_V' DATA_VERSION];
	else, file_name = vs;
	end
end
