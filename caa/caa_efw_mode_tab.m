function [t_start_save,t_stop_save,fdm_save]=caa_efw_mode_tab(db_s,start_epoch,delta_t,cli,var_list_s)
%caa_efw_mode_tab reads EFW FDM and extracts diffs
% [t_start_save,t_stop_save,fdm_save]=
% 	caa_efw_mode_tab(db_s,start_epoch,dt,cl_id,var_list_s)
%
...
while t_cur < stop_epoch
	[t_fdm,fdm_is] = caa_is_get(db_s,t_cur,dt,cli,'efw','FDM');
	...
	if ~isempty(t_fdm);
		...
		irf_log('dsrc',['No data at ' datestr(epoch2date(t_cur - dt),31)])
