...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 elseif strcmp(q,'ea'), % create E ascii files
  for ic=sc_list,
     ...
     eval(av_ssub('offset_comment=[''Damp='' num2str(Damp?) '', Da?p12='' num2str(Da?p12) '', Da?p34='' num2str(Da?p34) '', Da?p12p34='' num2str(Da?p12p34) ''\n'']',ic)); 
 ...
 elseif strcmp(q,'ps'), % create V_sc n ascii files
  for ic=sc_list,
     ...
     eval(av_ssub('if ~exist(''NVps?''), load mP NVps?;end',ic));
     if eval(av_ssub('exist(''NVps?'')',ic)),
       eval(av_ssub('number_of_points=size(NVps?,1);',ic));
