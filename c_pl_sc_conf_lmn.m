function h=c_pl_sc_conf_lmn(time,R1,R2,R3,R4,L,M,N);
%C_PL_SC_CONF_LMN   Plot the configuration of CLuster in LMN coordinates
%
%   h = C_PL_SC_CONF_LMN;
%   h = C_PL_SC_CONF_LMN(t);
%   h = C_PL_SC_CONF_LMN(t,n);
%   h = C_PL_SC_CONF_LMN(t,n,b);
%   h = C_PL_SC_CONF_LMN(t,l,m,n);
%   h = C_PL_SC_CONF_LMN(t,R1,R2,R3,R4,n);
...
if strcmp(action,'initialize'),
  ...
  h(4)=subplot(2,2,4);axis off;
  	ht=irf_pl_info(['c_pl_sc_conf_lmn() ' datestr(now)],gca,[0,1 ]); set(ht,'interpreter','none');
...
elseif strcmp(action,'plot'),
  ...
  if flag_L==0,
    b=irf_resamp(B,t); l=b(1,[2 3 4]);set(LHndl,'string',num2str(l,3));
  else
