...
case 'new_var_enter'
  ...
  c_4_v_int('new_var'); 
case 'new_var'
  var1=evalin('base',av_ssub(variable_str,1));
  var2=evalin('base',av_ssub(variable_str,2));
  var3=evalin('base',av_ssub(variable_str,3));
  var4=evalin('base',av_ssub(variable_str,4));
...
case 'initialize'
  ...
  h(1)=subplot(3,1,1);
  c_pl_tx(var1,var2,var3,var4,var_col);zoom on;
  av_pl_info(['c\_4\_v\_int() ' datestr(now)]); % add information to the plot
  ...
  ud.filter = uicontrol('style', 'edit', ...
    'string', '1', ...
    'callback', 'c_4_v_intdt'')', ...
    'backgroundcolor','white','units','normalized','position', [xp+0.15 yp 0.1 0.05]);
  
  uimenu('label','Auto &YLim','accelerator','y','callback','c_4_v_int(''autoY'')');
  uimenu('label','&Distance','accelerator','d','callback','c_4_v_int(''distance'')');
  uimenu('label','Click&Times','accelerator','t','callback','c_4_v_int(''click_times'')');
  uimenu('label','New&Variable','accelerator','v','callback','c_4_v_int(''new_var_enter'')');
...
case 'dt'
  ...
  if eval(get(ud.filter,'string'))<1,
    ...
    for ic=1:4,eval(av_ssub('x?=av_t_lim(var?,xl+[-20/Fs 20/Fs]);x?=av_tsfilt(x?,0,flim,Fs,5);',ic)),end
...
case 'click_times'
  ...
  if ic==0,
    set(gcf,'windowbuttondownfcn', 'c_4_v_int(''click_times'')');
    ic=1; 
  else, 
