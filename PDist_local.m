...
%     userData = [];
  ...
  methods
    ...
    %%% this is not inserted in PDist.m
    function PD = vd3v(obj,varargin)
      ...
      if doScpot
        ...
        E_minus(E_minus<0)= 0;
        E_plus(E_plus<0)= 0;
        if isfield(new_ancillary,'scpot')
          new_ancillary.scpot = {new_ancillary.scpot{:},'vd3v: data_corrected, depend not'};
        else
          new_ancillary.scpot = {'vd3v: data corrected, depend not'};
        end
      else
        E_minus = (obj.depend{1} - obj.ancillary.delta_energy_minus);
        E_plus = (obj.depend{1} + obj.ancillary.delta_energy_plus);
      ...
      switch obj.units
        ...
        otherwise
          error(sprintf('Units %s not supported.',old_units));
      end
      
      PD = obj;
      PD.data = PD.data.*vd3v*d3v_scale;
      PD.units = new_units;
      PD.ancillary = new_ancillary;
      PD.siConversion = num2str(str2num(PD.siConversion)/d3v_scale,'%e');
    end   
    ...
    function PD = red_flux(obj,varargin)
      % int(v f dv) performed on reduced 1D distribution      
      ...
      dv = diff(obj.ancillary.v_edges);
      v_plus = obj.ancillary.v_edges(2:end);
      v_minus = obj.ancillary.v_edges(1:(end-1));
      % int(vfdv) -> f[v^2/2]
      d_vel = (v_plus.^2 - v_minus.^2)/2; % 
      ...
      switch obj.units
        case 's/cm^4' 
          % m^4/s^4 = m^4/s^4 * cm^4/cm^4 = cm^4/s^4 * m^4/cm^4 = cm^4/s^4 * (10^-2)^4
          d3v_scale = 1/10^(-2*2);
          new_units = sprintf('1/cm^2s');
        case 's/m^4'  
          % m^4/s^4 = m^4/s^4 * m^4/m^4 = m^4/s^4 * m^4/m^4 = m^4/s^4 * (10^0)^4
          d3v_scale = 1/10^0;
          new_units = sprintf('1/m^2s');
          %new_units = '1/m^2s';
        case 's/km^4'  
          % m^4/s^4 = m^4/s^4 * km^4/km^4 = km^4/s^4 * m^4/km^4 = km^4/s^4 * (10^3)^4
          d3v_scale = 1/10^(3*2);
          new_units = sprintf('1/km^2s');
          %new_units = '1/km^2s';
      end
      
      PD = obj;
      PD.data = PD.data.*vdv*d3v_scale;
      PD.units = new_units;
      PD.siConversion = num2str(str2num(PD.siConversion)/d3v_scale,'%e');
    end   
    %%%
