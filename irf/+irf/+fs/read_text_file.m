%
% Reads file as text file and returns the rows in a 1D cell array.
%
%
% RETURN VALUE
% ============
% rowsCa
%       Column cell array of strings (one per row). All characters in the line
%       breaks have been removed so that it works for both Windows-style and
%       Unix-style text files (assuming linebreakRegexp has been set correctly).
%       NOTE: There will be a row after the last line break. The caller has to
%       decide whether this is a legitimate row.
% linebreakRegexp
%       Regular expression used as line break.
%
function [rowsCa] = read_text_file(filePath, linebreakRegexp)
...
%           CON: If want automatic detection of type of line break, then rowsCa is the proper format, not the
%                multi-row string.
...
% IMPLEMENTATION NOTE: The old implementation using textscan merged consequtive
% linebreaks (CR+LF) into one (i.e. bug). Therefore not using it.
...
rowsCa = strsplit(...
  char(bytesArray(:)'), linebreakRegexp, ...
