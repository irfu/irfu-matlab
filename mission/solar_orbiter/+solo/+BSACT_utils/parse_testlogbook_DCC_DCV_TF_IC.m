...
function metadataList = parse_testlogbook_DCC_DCV_TF_IC(rowStrList, dataType)
    ...
    % -----------------------------------------------------------------
    % NoDataPrefix : Any row before the first "Header2" row. (Row which will be ignored.)
    % NoDataSuffix : Any row after  the last  "Test"    row. (Row which will be ignored.)
...
function CTableMetadata = parse_test_row_DCV_TF_IC(rowStr)
    ...
    % NOTE: Should assign NaN if not found. Can therefore not use map_regex_to_values.
