...
function hAxesArray = plot_HK(filePath)
    ...
    %=====================================
    for i = 1:numel(ZV_SINGLE_SCALAR_LIST)
    ...
    %==================================================================
    for i = 1:numel(ZV_TRIPLET_SCALAR_LIST)
        Sp.add_panel_time_series3_HK(D, ZV_TRIPLET_SCALAR_LIST{i});
    end
    ...
    %==================================================================
%     hBitArray = [];
    for i = 1:numel(ZV_BIT_FLAG_LIST)
        zvName = ZV_BIT_FLAG_LIST{i};
        Sp.add_panel_plot_bit_series(zvName, Epoch, D.data.(zvName).data, zvName, BIT_PANEL_HEIGHT);
    end
...
function zvNameTripletCa = ZV_name_triplet(zvNamePattern)
    zvNameTripletCa = {};
    for iAnt = 1:3
        zvNameTripletCa{iAnt} = sprintf(zvNamePattern, iAnt);
    end
end
