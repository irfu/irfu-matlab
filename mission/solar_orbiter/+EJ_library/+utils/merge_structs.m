...
% The field names must not overlap. Not recursive.
%
% NOTE: Implemented by creating one combined call struct(fieldName1, fieldValue1, ...).
%
%
...
% S12    : Struct ROW array (regardless of sizes of s1, s2).
%
%
% NOTE: Can be used to effectively add a field to an existing struct array.
% NOTE: EJ_library.utils.add_struct_to_struct is similar.
%
%
...
% s1=struct('a', {1,2,3,4}); s2=struct('b', {2,3,4,6}', 'c', {4,5,6,7}'); s=bicas.utils.merge_structs(s1, s2)
    ...
    if ~isvector(S1) || ~isvector(S2)
        error('merge_structs:Assertion', 'At least one of the structs is not a vector array.')
    end
    if ~(isscalar(S1) || isscalar(S2))
        if numel(S1) ~= numel(S2)    % NOTE: Only works since S1,S2 are vectors.
            error('merge_structs:Assertion', 'Structs are arrays of different non-scalar size.')
        end
    end
    ...
    S12 = struct(structArgsList{:});    % Gives error if reuses same fieldname.
end
