...
% ARGUMENTS
% =========
% fillVal    : Scalar numeric. Value used for components in y not determined by y1 and y2.
% x1Ca, x2Ca : Size Nx1 cell arrays of 1D numeric arrays.
...
%       Values come from y1, and y2, and fillVal. In case of collisions, then y2 takes precedence over y1.
%
%
...
function [xCa, y] = merge_coordinated_arrays(fillVal, x1Ca, y1, x2Ca, y2)
    ...
    % IMPLEMENTATION NOTE: Every N-dimensional array can in principle be seen as an infinite-dimensional array, with
    % size=1 for dimensions >=N+1. These higher dimensions do not have coordinates associated with them. A
    % zero-dimensional array is thus equivalent to a scalar, without any coordinates, and it is distinct from e.g.
    % a 2D array of size 1x1, which would have coordinate values for the first and second dimension. Similarily, an
    % empty array, e.g. 0x0, therefore has at least dimension.

    % ASSERTIONS
    assert(isscalar(fillVal))
    nDims = EJ_library.assert.sizes(x1Ca(:), [-1], x2Ca(:), [-1]);
    ...
    for i = 1:nDims
        % ASSERTIONS
        % There is no known fundamental reason to not permit zero coordinates on axis. The code is just not vetted for
        % that case yet.
        assert(length(x1Ca{i}) >= 1)
        assert(length(x1Ca{i}) >= 2)
        ...
        EJ_library.assert.number_set(x1Ca{i})
        EJ_library.assert.number_set(x2Ca{i})
    ...
    % NOTE: Empirically, subsasgn can reliably crash MATLAB when y=NaN, y1=[], and S1.subs={}, i.e. zero-dimensions
