...
function [sizeArray] = distribute_segments(totalSize, fixedSizeArray, weightArray)
    ...
    % ASSERTIONS: Whether to assert not using negative values. Algorithm should work with negative values, but unsure
    ...
    % ASSERTIONS
    assert(totalWeights ~= 0)    % NOTE: Algorithm is undetermined without this (multiple solutions; practically divide-by-zero.)
    if DISALLOW_NEGATIVE_SIZES_WEIGHTS
        assert(totalSize >= 0)
        assert(all(fixedSizeArray >= 0))
        assert(all(weightArray >= 0))
        assert(totalFixedSize < totalSize, 'Must have sum(fixedSizeArray) < totalSize, but that is not true.')
    end
