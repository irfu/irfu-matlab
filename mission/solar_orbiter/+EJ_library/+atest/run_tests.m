...
function run_tests(testList)
    ...
    %
    % PROPOSAL: On failure, print function call that generated failure so that
    %           user can call manually from CLI.
    ...
    for iTest = 1:nTests
        ...
        if ~TestData.success
            ...
            EJ_library.utils.print_variable_recursively(...
                '    Parameters', TestData.Parameters, Settings);
            EJ_library.utils.print_variable_recursively(...
                '    Result',     TestData.Result,     Settings);
            %keyboard
