...
function newTt2000 = convert_N_to_1_SPR_Epoch( oldTt2000, nSpr, freqHzWithinRecords )
    ...
    if ~iscolumn(oldTt2000)
        error('convert_N_to_1_SPR_Epoch:Assertion:IllegalArgument', 'Argument is not a column vector')   % Right ID?
    elseif ~isa(oldTt2000, 'int64')
        error('convert_N_to_1_SPR_Epoch:Assertion:IllegalArgument', 'Argument has the wrong class.')   % Right ID?
    end
    if numel(nSpr) ~= 1
        error('convert_N_to_1_SPR_Epoch:Assertion:IllegalArgument', 'nSpr not scalar.')
    elseif size(freqHzWithinRecords, 1) ~= size(oldTt2000, 1)
        error('convert_N_to_1_SPR_Epoch:Assertion:IllegalArgument', 'freqWithinRecords and oldTt2000 do not have the same number of rows.')
    end
