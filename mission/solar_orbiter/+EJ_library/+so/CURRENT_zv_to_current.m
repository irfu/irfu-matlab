...
% Therefore has some extra features to handle the quirks of the SOLO_L1_RPW-BIA-CURRENT dataset format.
%
% NOTE: "CURRENT" refers to DATASET_ID=SOLO_L1_RPW-BIA-CURRENT
% NOTE: Does not try to calibrate or convert units. The output bias is of the same type as the bias input.
% NOTE: See CURRENT_zv_to_current_interpolate.
% 
%
% ALGORITHM / EFFECT
% ==================
% (1) Removes timestamp+bias when zvIBIASx1==NaN, since these occur in the dataset format (meaning that bias
% current is set on another antenna).
% (2) Removes  duplicate bias settings (successive non-NaN data points with the same timestamp and bias setting).
% Returns a flag value for whether this has happened or not. This kind of data is due to a bug(?) in
% SOLO_L1_RPW-BIA-CURRENT and is therefore flagged rather than asserted not to happen so that the caller can select
% whether to give error/warn/accept, whether to use this as mitigation or not.
% --
% NOTE: Does NOT remove bias settings that set the bias to the preceeding value at a later timestamp ("unnecessary later
% bias settings").
%
%
% ARGUMENTS
% =========
% t1        : 1D vector. Increasing (sorted), not necessarily monotonically. Time. Time representation unimportant as
%             long as increases with time. Can be TT2000.
...
%                     Found duplicate timestamps, with the SAME bias current.
%                     NOTE: Bias current is still unambiguous in this case and the function is designed to handle this
%                     case.
...
%     zvIBIASx1 = double(zvIBIASx1);
    ...
    % Remove indices at which CURRENTS (not Epoch) are NOT NaN, i.e. which provide actual bias values on this antenna.
