...
% varargin
%       Settings passed to EJ_library.utils.interpret_settings_args(). See
%       implementation.
%
%
%
...
% this into account by internally converting the zVar variable back to the right class and size.
    ...
    %
    EJ_library.assert.castring_set(zvNameAllCa1)
    EJ_library.assert.castring_sets_equal(zvNameAllCa1, zvNameAllCa2)
    ...
    for i = 1:length(dataobj_Variables(:,1))
        ...
        specifiedMatlabClass   = EJ_library.cdf.convert_CDF_type_to_MATLAB_class(...
        ...
        for iZvaOfZvDataType = 1:length(ZVA_OF_ZV_DATA_TYPE_NAME_LIST)
            ...
            if isempty(iDovafRow)
                % CASE: The current zVariable does not have this attribute
                % (varAttrName).
                continue
            ...
            elseif ~strcmp(specifiedMatlabClass, class(zvaValue))
                msg = sprintf(...
                    ['Found VariableAttribute %s for CDF variable "%s"', ...
...
% RETURN VALUES
% =============
% zvValue       : The variable that should be passed to spdfcdfwrite. Can be (1)
...
% Modify zvValue so that it can be passed to spdfcdfwrite and be interpreted
% correctly.
%
% ARGUMENTS
% =========
        ...
        if Settings.strictNumericZvSizePerRecord || ...
          (Settings.strictEmptyNumericZvSizePerRecord && (nRecords == 0))
            ...
            if ~isequal(...
                ...
                sizePerRecordStr       = ['[', ...
                    strjoin(EJ_library.str.sprintf_many('%i', sizePerRecord), ', '), ...
                    ']'];
                specifiedSizePerRecordStr = ['[', ...
                    strjoin(EJ_library.str.sprintf_many('%i', specifiedSizePerRecord), ', '), ...
                    ']'];
