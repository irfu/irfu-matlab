...
% destDir
%       Path to directory of the RCT file to be created (compliant filename
%       will be generated by the function). NOTE: Any pre-existing destination
...
% (4) it is easier to edit if the RCT is extended with more data by changing
%     the format (in particular more transfer functions, by some multiple).
...
Dsfn        = solo.adm.dsfn.DatasetFilename.parse_filename(rctFilename);
assert(~isempty(Dsfn), 'Can not parse filename "%s".', rctFilename)

gact = rctFilename(1:end-4);
DataObj.GlobalAttributes.CALIBRATION_TABLE   = {gact};

DataObj.GlobalAttributes.CALIBRATION_VERSION = {Dsfn.versionStr};
...
DataObj.GlobalAttributes.Datetime     = Dsfn.timeIntervalStr;
