...
% IMPLEMENTATION NOTE: Filenaming convention is not followed here. Not sure how to comply with it either (which
% receiver should the BIAS RCT specify?).
% switch pipelineId
%     case {'ROC-SGSE', 'RGTS'}
%         % Sets name analogous to other teams' RCTs. Seems incorrect.
%         destFilename = sprintf('ROC-SGSE_CAL_RPW_BIAS_V%s.cdf', datestr(clock, 'yyyymmddHHMM'));
%     case 'RODP'
        destFilename = sprintf(    'SOLO_CAL_RPW_BIAS_V%s.cdf', datestr(clock, 'yyyymmddHHMM'));
%     otherwise
%         error('create_RCT:Assertion', 'Can not interpret "pipelineId=%s', pipelineId)
% end
...
RctZvL = add_RCT_zvars_L(RctZvL, int64(0), [-1.72313e-09, 4.83034e-08, 4.87629e-08]', [-1.98004e-15, -1.97993e-15, -1.98017e-15]', ...
...
if 0
% TEST: Copy-pasted. Can be deleted.
RctZvL = add_RCT_zvars_L(RctZvL, int64(1), [-1.72313e-09, 4.83034e-08, 4.87629e-08]', [-1.98004e-15, -1.97993e-15, -1.98017e-15]', ...
    create_tfc_zvar_record(...
        'DC_single', {[-5.009e20,  8.148e14, -1.041e10],                      [8.556e21, 2.578e17, 2.042e12, 8.238e05, 1]}, ...
        'DC_diff',   {[-2.311e23, -1.009e18,  2.664e11],                      [2.329e23, 4.411e18, 7.344e12, 3.868e06, 1]}, ...
        'AC_lg',     {[-2.287e18, -1.365e18, -1.946e12],                      [1.348e19, 2.68e17 , 4.828e12, 3.85e06,  1]}, ...
