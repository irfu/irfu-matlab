MATLAB package for creating RTDPs. See
bicas.tools.rtdp.create_RCS_test_data_package().


==========================================================
EXAMPLE DIRECTORY TREE FOR AN RCS TEST DATA PACKAGE (RTDP)
==========================================================

TESTDATA_RODP_BICAS_V9.0.0A
├── L1R-L2_LFR-SBM1-CWF-E
│   ├── expected_outputs
│   │   ├── manifest.txt
│   │   └── solo_L2_rpw-lfr-sbm1-cwf-e_20210715T234148-20210715T235548_V01.cdf
│   └── inputs
│       ├── manifest.txt
│       ├── solo_HK_rpw-bia_20210715_V05.cdf
│       ├── solo_L1R_rpw-lfr-sbm1-cwf-e-cdag_20210715T234148-20210715T235548_V02.cdf
│       └── solo_L1_rpw-bia-current-cdag_20210701-20210731_V34.cdf
├── L1R-L2_LFR-SBM2-CWF-E
│   ├── expected_outputs
│   │   ├── manifest.txt
│   │   └── solo_L2_rpw-lfr-sbm2-cwf-e_20220922T134335-20220922T154536_V01.cdf
│   └── inputs
│       ├── manifest.txt
│       ├── solo_HK_rpw-bia_20220922_V06.cdf
│       ├── solo_L1R_rpw-lfr-sbm2-cwf-e-cdag_20220922T134335-20220922T154536_V01.cdf
│       └── solo_L1_rpw-bia-current-cdag_20220901-20220930_V37.cdf
├── L1R-L2_LFR-SURV-CWF-E
│   ├── expected_outputs
│   │   ├── manifest.txt
│   │   └── solo_L2_rpw-lfr-surv-cwf-e_20200213_V01.cdf
│   └── inputs
│       ├── manifest.txt
│       ├── solo_HK_rpw-bia_20200213_V06.cdf
│       ├── solo_L1R_rpw-lfr-surv-cwf-e-cdag_20200213_V10.cdf
│       └── solo_L1_rpw-bia-current-cdag_20200211-20200229_V02.cdf
├── L1R-L2_LFR-SURV-SWF-E
│   ├── expected_outputs
│   │   ├── manifest.txt
│   │   └── solo_L2_rpw-lfr-surv-swf-e_20200213_V01.cdf
│   └── inputs
│       ├── manifest.txt
│       ├── solo_HK_rpw-bia_20200213_V07.cdf
│       ├── solo_L1R_rpw-lfr-surv-swf-e-cdag_20200213_V10.cdf
│       └── solo_L1_rpw-bia-current-cdag_20200211-20200229_V03.cdf
├── L1R-L2_TDS-LFM-CWF-E
│   ├── expected_outputs
│   │   ├── manifest.txt
│   │   └── solo_L2_rpw-tds-lfm-cwf-e_20200225_V01.cdf
│   └── inputs
│       ├── manifest.txt
│       ├── solo_HK_rpw-bia_20200225_V06.cdf
│       ├── solo_L1R_rpw-tds-lfm-cwf-e-cdag_20200225_V07.cdf
│       └── solo_L1_rpw-bia-current-cdag_20200211-20200229_V04.cdf
├── L1R-L2_TDS-LFM-RSWF-E
│   ├── expected_outputs
│   │   ├── manifest.txt
│   │   └── solo_L2_rpw-tds-lfm-rswf-e_20200409_V01.cdf
│   └── inputs
│       ├── manifest.txt
│       ├── solo_HK_rpw-bia_20200409_V08.cdf
│       ├── solo_L1R_rpw-tds-lfm-rswf-e-cdag_20200409_V08.cdf
│       └── solo_L1_rpw-bia-current-cdag_20200401-20200430_V03.cdf
├── readme.txt
└── release_notes.txt
