...
% "BUGS:"
% =======
% - Sets GlobalAttributes.Generation_date in local time.
% - Calls derive_output_dataset_GlobalAttributes for ALL input dataset and uses the result for ALL output datasets.
%   ==> If there are multiple output datasets based on different sets of input datasets, then the results might be wrong.
%   Should ideally be run on the exact input datasets (~EIn PDs) used to produce a specific output dataset.
%
...
%inputFilenameList = {};
%InputDatasetsMetadataMap = containers.Map;
...
%============================================================================================
% Iterate over all OUTPUT process data field names (~zVariables) : Modify MasterDataobj.data
%============================================================================================
% Modify copy of struct?!
fieldNameList = fieldnames(ProcessData);
for iPdFieldName = 1:length(fieldNameList)
    zVariableName = fieldNameList{iPdFieldName};
    irf.log('n', sprintf('Converting process data struct field "%s" into CDF zVariable.', zVariableName))
    
    matlabClass = bicas.utils.convert_CDF_type_to_MATLAB_class(MasterDataobj.data.(zVariableName).type, 'Permit MATLAB classes');
    MasterDataobj.data.(zVariableName).data = cast(ProcessData.(zVariableName), matlabClass);
end



MasterDataobj.GlobalAttributes.Software_name       = CONSTANTS.C.SWD_identification.name;
MasterDataobj.GlobalAttributes.Software_version    = CONSTANTS.C.SWD_release.version;
MasterDataobj.GlobalAttributes.Calibration_version = CONSTANTS.C.Calibration_version;             % "Static"?!!
MasterDataobj.GlobalAttributes.Generation_date     = datestr(now, 'yyyy-mm-ddTHH:MM:SS');         % BUG? Assigns local time, not UTC!!! ROC DFMD does not mention time zone.
MasterDataobj.GlobalAttributes.Logical_file_id     = logical_file_id(datasetId, GlobalAttributesSubset.Test_Id, GlobalAttributesSubset.Provider);
MasterDataobj.GlobalAttributes.Parents             = GlobalAttributesSubset.Parents;
MasterDataobj.GlobalAttributes.Parent_version      = GlobalAttributesSubset.Parent_version;
MasterDataobj.GlobalAttributes.Data_version        = CONSTANTS.C.OUTPUT_CDF.Data_version;         % ROC DFMD says it should be updated in a way which can not be automatized?!!!
MasterDataobj.GlobalAttributes.Provider            = GlobalAttributesSubset.Provider;             % ROC DFMD contradictive if it should be set.
if CONSTANTS.C.OUTPUT_CDF.SET_Test_id
    MasterDataobj.GlobalAttributes.Test_id         = GlobalAttributesSubset.Test_Id;              % ROC DFMD says that it should really be set by ROC.
end
%MasterDataobj.GlobalAttributes.SPECTRAL_RANGE_MIN
%MasterDataobj.GlobalAttributes.SPECTRAL_RANGE_MAX
%MasterDataobj.GlobalAttributes.TIME_MIN
%MasterDataobj.GlobalAttributes.TIME_MAX
%MasterDataobj.GlobalAttribute CAVEATS ?!! ROC DFMD hints that value should not be set dynamically. (See meaning of non-italic black text for global attribute name in table.)
...
% PROPOSAL: "Validate" the CDF file read here / assertions.
...
bicas.utils.assert_strings_equal(...
    ...
    'The input CDF file''s stated DATASET_ID does not match the value expected for the S/W mode.')
bicas.utils.assert_strings_equal(...
    CONSTANTS.C.INPUT_CDF_ASSERTIONS.STRICT_Skeleton_version, ...
    {file_Skeleton_version, C_input.skeleton_version_str}, ...
    'The input CDF file''s stated Skeleton_version does not match the value expected for the S/W mode.')
