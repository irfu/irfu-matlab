...
for i = 1:length(SwmInfo.inputsList)
  ...
  cdfDsid = InputDataset.Ga.Dataset_ID{1};

  if ~strcmp(cdfDsid, SwmInfo.inputsList(i).dsid)
    [settingValue, settingKey] = Bso.get_fv(...
      'INPUT_CDF.GA_DSID_MISMATCH_POLICY');
    anomalyDescrMsg = sprintf(...
      ['The input CDF dataset''s stated DSID does', ...
      ' not match value expected from the S/W mode.\n', ...
      '    File: %s\n', ...
      '    Global attribute InputDataset.Ga.Dataset_ID{1} : "%s"\n', ...
      '    Expected value:                                : "%s"\n'], ...
      inputFilePath, cdfDsid, SwmInfo.inputsList(i).dsid);
...
for iOutputCdf = 1:length(SwmInfo.outputsList)
  ...
  GaSubset = bicas.ga.get_output_dataset_GAs(...
    ...
    irf.fs.get_name(outputFilePath), SwmOutputDataset.dsid, ...
    Bso, L);
