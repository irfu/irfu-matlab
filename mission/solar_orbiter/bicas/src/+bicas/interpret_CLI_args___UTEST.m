...
classdef interpret_CLI_args___UTEST < matlab.unittest.TestCase
    ...
    methods(Test)
        ...
        function test0(testCase)
            ...
            %=========
            bicas.interpret_CLI_args___UTEST.test(testCase, ...
                {'swmode', '--in', 'infile', '--out', 'outfile'}, ...
                'S/W mode', 'swmode', [], [], [], {{}, {}}, {{'in', 'out'}, {'infile', 'outfile'}});
            
            % + extra
            bicas.interpret_CLI_args___UTEST.test(testCase, ...
                {'swmode', '--in', 'infile', '--config', 'configfile', '--out', 'outfile'}, ...
                'S/W mode', 'swmode', 'configfile', [], [], {{}, {}}, {{'in', 'out'}, {'infile', 'outfile'}});

            % Multiple input/output file arguments.
            bicas.interpret_CLI_args___UTEST.test(testCase, ...
                {'swmode', '--in1', 'infile1', '--out1', 'outfile1', '--in2', 'infile2', '--out2', 'outfile2'}, ...
                'S/W mode', 'swmode', [], [], [], {{}, {}}, ...
                {{'in1', 'out1', 'in2', 'out2'}, ...
            ...
            bicas.interpret_CLI_args___UTEST.test_EXC(testCase, ...
                {'--in', 'infile', '--out', 'outfile', 'swmode'});
            bicas.interpret_CLI_args___UTEST.test_EXC(testCase, ...
                {'--in', 'infile', 'swmode', '--out', 'outfile'});
             
            % S/w mode in the wrong place.
            bicas.interpret_CLI_args___UTEST.test_EXC(testCase, ...
                {'--in', 'infile', 'swmode', '--config', 'configfile', '--out', 'outfile'});

            % S/w mode in TWO places (one correct). (Tested due to how algorithm works.)
            bicas.interpret_CLI_args___UTEST.test_EXC(testCase, ...
                {'swmode1', '--in', 'infile', 'swmode2', '--config', 'configfile', '--out', 'outfile'});
    ...
    methods(Static, Access=private)
        ...
        function test(testCase, ...
                cliArgList, functionalityMode, swModeArg, configFile, ...
            ...
            expOutput = struct(...
                ...
                'ModifiedSettingsMap',    EJ_library.utils.create_containers_Map(...
                    'char', 'char', ModifiedSettingsMap{1:2}), ...
                'SpecInputParametersMap', EJ_library.utils.create_containers_Map(...
