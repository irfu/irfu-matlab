%
% Collection of utility functions used by bicas.proc.L1L2.cal to reduce its size. Only
...
classdef cal_utils
    ...
    methods(Static)
        ...
        function ModifTabTf = extrapolate_tabulated_TF_to_zero_Hz(TabTf)
            ...
            signZ0   = sign(real(Z1));
            assert(signZ0 ~= 0, ...
                'BICAS:cal_utils:extrapolate_tabulated_TF_to_zero_Hz:FailedToReadInterpretRCT:Assertion', ...
...
%             end
        ...
        function assert_iBlts(iBlts)
            assert(ismember(iBlts, [1:5]), ...
                'BICAS:cal_utils:IllegalArgument:Assertion', ...
