...
classdef dc___UTEST < matlab.unittest.TestCase
  ...
  methods(Test)
    ...
    %
    function test_calibrate_voltage_1xBLTS___CWF_NaN(T, TF_METHOD)
      Bso = T.get_simple_TF_BSO(TF_METHOD);
      ...
      T.assertEqual(actVoltageAvolt, expVoltageAvolt, AbsTol=1e-14)
    end



    % Test SWF NaN data.
    function test_calibrate_voltage_1xBLTS___SWF_NaN(T, TF_METHOD)
      Bso = T.get_simple_TF_BSO(TF_METHOD);
    ...
    function test_get_SSID_SDID_arrays(T)
      S = T.S;
      ...
      % Tests
      %=======
      T.test_get_SSID_SDID_arrays_helper(...
    ...
    function test_get_VSIB_5xBLTS___CWF(T)
      ...
      T.assertEqual(actBltsVsibAr, expBltsVsibAr)
    end



    function test_get_VSIB_5xBLTS___SWF(T)
  ...
  methods(Access=private)
    ...
    function Bso = get_simple_TF_BSO(T, tfMethod)
    ...
    function test_get_SSID_SDID_arrays_helper(...
        T, bdmFpa, dlrFpa, expBltsSsidArray, expBltsSdidArray)
      ...
      T.assertEqual(size(actBltsSsidArray), size(actBltsSdidArray))
      T.assertEqual(size(actBltsSsidArray, 2), bicas.const.N_BLTS)
      T.assertEqual(class(actBltsSsidArray), 'uint8')
      T.assertEqual(class(actBltsSdidArray), 'uint8')

      T.assertEqual(actBltsSsidArray, uint8(expBltsSsidArray))
      T.assertEqual(actBltsSdidArray, uint8(expBltsSdidArray))
    end
