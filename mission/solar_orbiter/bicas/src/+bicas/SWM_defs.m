...
classdef SWM_defs
    ...
    %         varies, but the arguments returned by bicas.SWM_defs must be the same.
    ...
    %       (1) SWM_defs (constructor), or (2) the call in execute_SWM.
    %   PROPOSAL: All arguments which are known at the time bicas.SWM_defs
    %       constructor is called, should receive values there.
    %       ==> ~As many as possible.
    %       CON: bicas.SWM_defs not really meant to set production function arguments.
    %       CON: Makes SWM_defs harder to initialize (outside of BICAS).
...
%     end
    ...
    methods(Access=public)
        ...
        % 
        function obj = SWM_defs(SETTINGS, L)
            ...
            HK_INPUT_DEF = bicas.SWM_defs.def_input_dataset(...
                'in_hk', 'SOLO_HK_RPW-BIA', 'HK_cdf');
            CUR_INPUT_DEF = bicas.SWM_defs.def_input_dataset(...
            ...
            for iInputLevel = 1:numel(inputDatasetLevelList)
                ...
                for iSwm = 1:length(LFR_SW_MODE_DATA)
                    ...
                    SciInputDef = bicas.SWM_defs.def_input_dataset(...
                        'in_sci', ...
                        strmod('SOLO_<InLvl>_RPW-LFR-<SBMx/SURV>-<C/SWF><I-E>'), ...
                        'SCI_cdf');

                    SciOutputDef = bicas.SWM_defs.def_output_dataset(...
                        'out_sci', ...
                    ...
                    SwModeList(end+1) = bicas.SWM_defs.def_swmode(...
            ...
            if SETTINGS.get_fv('SW_MODES.L2-L2_CWF-DSR_ENABLED')
                SciInputDef = bicas.SWM_defs.def_input_dataset(...
                    'in_sci', ...
            ...
            if SETTINGS.get_fv('SW_MODES.L2-L3_ENABLED')
                ...
                EfieldOutputDef = bicas.SWM_defs.def_output_dataset(...
                    'out_efield', ...
                ...
                ScpotOutputDef = bicas.SWM_defs.def_output_dataset(...
                    'out_scpot', ...
                ...
                DensityOutputDef = bicas.SWM_defs.def_output_dataset(...
                    'out_density', ...
                ...
                EfieldDsrOutputDef = bicas.SWM_defs.def_output_dataset(...
                    'out_efield_dsr', ...
                ...
                ScpotDsrOutputDef = bicas.SWM_defs.def_output_dataset(...
                    'out_scpot_dsr', ...
                ...
                DensityDsrOutputDef = bicas.SWM_defs.def_output_dataset(...
                    'out_density_dsr', ...
    ...
    methods(Static, Access=private)
        ...
        % NOTE: Name dangerously similar to "bicas.SWM_defs".
        function Def = def_swmode(...
            ...
            % ASSERTIONS
            %============
            bicas.SWM_defs.assert_SW_mode_CLI_option(Def.cliOption)
            bicas.SWM_defs.assert_text(              Def.swdPurpose)
        ...
        function Def = def_input_dataset(...
            ...
            bicas.SWM_defs.assert_SIP_CLI_option(Def.cliOptionHeaderBody)
        ...
        function Def = def_output_dataset(...
            ...
            bicas.SWM_defs.assert_SW_mode_CLI_option(Def.cliOptionHeaderBody)
