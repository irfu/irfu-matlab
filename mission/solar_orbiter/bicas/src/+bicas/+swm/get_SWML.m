...
SwmArray = get_SWMs_L1_L1R_to_L2(Bso);
...
function SwmArray = get_SWMs_L1_L1R_to_L2(Bso)

% Arrays with constants.
% {1} = S/w modes (science) L1R-->L2
% {2} = S/w modes (science) L1 -->L2
inputDatasetLevelCa = {'L1R'};
inputDashECa        = {'-E'};
swmSuffixCa         = {''};
swmPurposeAmendmCa  = {''};

if Bso.get_fv('SWM.L1-L2_ENABLED')
  inputDatasetLevelCa{end+1} = 'L1';
  inputDashECa{end+1}        = '';
  swmSuffixCa{end+1}         = '_L1';
  swmPurposeAmendmCa{end+1}  = ' UNOFFICIAL wrt. ROC.';
end
...
strmodg = @(s, iInputLevel) bicas.utils.strrep_many(s, ...
  '<InLvl>',              inputDatasetLevelCa{iInputLevel}, ...
  '<I-E>',                inputDashECa{iInputLevel}, ...
  '<SWM suffix>',         swmSuffixCa{iInputLevel}, ...
  '<SWM purpose amendm>', swmPurposeAmendmCa{iInputLevel});
...
LFR_SWM_DATA = struct(...
  ...
  'outputSkeletonVersion', repmat({L2_LFR_TDS_SKT_VERSION}, 1, 4));
TDS_SWM_DATA = struct(...
  'CWF_RSWF',              {'CWF', 'RSWF'}, ...
  'outputSkeletonVersion', repmat({L2_LFR_TDS_SKT_VERSION}, 1, 2));



SwmArray = bicas.swm.SoftwareMode.empty(0, 1);
% Iterate over [L1R] (one component), or [L1, L1R]...
for iInputLevel = 1:numel(inputDatasetLevelCa)
  ...
  for iSwm = 1:numel(TDS_SWM_DATA)
    ...
    if strcmp(TDS_SWM_DATA(iSwm).CWF_RSWF, 'RSWF') ...
        && strcmp(inputDatasetLevelCa{iInputLevel}, 'L1')
      % CASE: TDS RSWF
