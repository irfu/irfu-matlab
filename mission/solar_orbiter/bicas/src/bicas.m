%
% Class which contains the main/top-level MATLAB function bicas.main() that
% launches BICAS.
%
...
classdef bicas
  ...
  methods(Static)
    ...
    % BICAS's main/top-level MATLAB function, which is called by no other
    % MATLAB code during nominal use at ROC. It is intended to be wrapped in,
    ...
    % To prevent the duplication of documentation, comments in the source code
    % tries to only cover subjects important for understanding the
    % implementation and information not already present in other documentation
    % text files.
    %
    %
    % ARGUMENTS
    % =========
    % varargin
    %       This function expects exactly the CLI arguments submitted to the
    ...
    % - The parameter syntax may contain additional unofficial parameters,
    %   which are useful for development/debugging, but which are still
    %   compatible with the RCS ICD.
    ...
    %
    % IMPLEMENTATION NOTE: This code does not quit/exit using the MATLAB
    % function "quit" since that always exits all of MATLAB which is
    % undesirable when developing code from in the MATLAB IDE. The function
    % returns the error code to make it possible for the bash wrapper to quit
    ...
    % 2) prints all messages intended for BICAS' final, actual stdout (as
    %    produced by the bash wrapper) to stdout but with a prefix so they can
    %    be filtered out by the calling wrapper bash script.
    ...
    function errorCode = main( varargin )
      ...
      %
      % PROPOSAL: Put a summarized version of CLI syntax in "bicas --help"
      %           (something simpler than the SWD).
      %   PRO: Useful when SWD becomes big and complex.
      %
      % PROPOSAL: Split up the main_without_error_handling() function into
      %           several (testable) functions.
      ...
      try
        ...
        bicas.main_without_error_handling(varargin(:), L);
        ...
        try
          ...
          [msgRecursive, errorCode] = bicas.recursive_exception_msg(Exception1);
    ...
    function [msg, errorCode] = recursive_exception_msg(Exception)
      ...
      for iCause = 1:numel(Exception.cause)
        ...
        recursiveMsg = bicas.recursive_exception_msg(Exception.cause{iCause});
    ...
    function main_without_error_handling(cliArgumentsCa, L)
      ...
      switch(CliData.bfmid)
        case 'VERSION_BFM'
          bicas.print_version(Swml)

        case 'IDENTIFICATION_BFM'
          bicas.print_identification(Swml)

        case 'SWD_BFM'
          bicas.print_SWD(Swml)

        case 'HELP_BFM'
          bicas.print_help(Bso)

        case 'SWM_BFM'
          %==========================
          % CASE: Process a S/W mode
          %==========================
          bicas.process_SWM(Swml, CliData.swmArg, CliData.SipMap, Bso, L)
    ...
    function print_version(Swml)
      ...
      % rather than the in the BICAS constants since the RCS ICD specifies that
      % it should be that specific version. This is in principle an inefficient
      % imlementation but it should also be more reliable.
    ...
    function print_help(Bso)
      ...
      bicas.stdout_print(bicas.sprint_constants());
    ...
    function process_SWM(Swml, swmArg, SipMap, Bso, L)
      ...
      % needed.
      rctDir       = bicas.read_env_variable(Bso, L, ...
        'ROC_RCS_CAL_PATH',    'ENV_VAR_OVERRIDE.ROC_RCS_CAL_PATH');
      masterCdfDir = bicas.read_env_variable(Bso, L, ...
      ...
      NsoTable = bicas.NsoTable.read_file_validated(...
        nsoTablePath, bicas.const.qrc.Q.ALL_QRCID_AR);
    ...
    % Read environment variable, but allow the value to be overriden by a
    % settings variable.
