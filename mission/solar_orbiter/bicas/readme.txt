...
List last updated 2016-11-10.

* Reading CDF files.:
    - BICAS does not check explicitly if an input CDF file, or master CDF file appears to be of the right type of dataset (dataset ID) before trying
      to process the information. NOTE: Many input datasets (i.e. NOT generated by BICAS) have not set the CDF Global Attributes needed to make
      this check.
...
* Writing CDF files:
    - zVariable IBIAS1-IBIAS3, DELTA_PLUS_MINUS, SAMP_DTIME, L1_REC_NUM are set to fill values.
