...
####################################
This section lists known current limitations. It is likely not complete or updated.
Most or all of these limitations represent features that should be implemented eventually.

List last updated 2016-10-13.

- Output CDF files have NaN instead of fill/pad values. (Fill/pad values for floating-point values are however read correctly from input CDF files.)
- Mux/demux modes 5-7 have not been implemented.
- Voltage calibration only uses proportionality factors, i.e.:
    - Transfer functions are not used.
    - No voltage offsets due to bias currents are added.
- zVariable IBIAS1-IBIAS3, DELTA_PLUS_MINUS are set to fill values.
- LFR: zVariables QUALITY_BITMASK, QUALITY_FLAG are set to fill values with the right size when the source dataset has empty zVariables
  (instead of copying).
- LFR: Algorithms use ACQUISITION_TIME for time rather than Epoch.
- Global attributes which should not be inherited from master CDF files are not set.
- CDF Epoch's variable attributes SCALEMIN, SCALEMAX are not set correctly.
- Not all modes described in the S/W descriptor necessarily work in practise, due to ongoing development.
- The conversions between tt2000 (Epoch), ACQUISITION_TIME and UTC have not been verified.x
- Many combinations of input dataset CDF zVariables are untested even if they should work in theory.
- BICAS does not check explicitly if an input CDF file, or master CDF file appears to be of the right type of dataset (dataset ID).
(- BICAS can not produce CDF files with zVariables with zero records.)

NOTE: Input CDF files (datasets) may have incorrect, or partially incorrect Epoch values, which translate into faulty Epoch values in BICAS' output CDF
files. This is due to known limitations in LESIA's (?) early production of test datasets (and BICAS can not compensate for this). LESIA prescribes that datasets
contain an extra time zVariable ACQUISITION_TIME that is used in parallel to Epoch as a temporary solution.
