...
RCS ICD                    = Document ROC-TST-GSE-ICD-00023-LES, "RPW Calibration Software ICD Documentation".
...
* Processing:
    ...
    - LFR: Algorithms use ACQUISITION_TIME for time rather than Epoch (for comparing HK data time stamps to SCI timestamps).
...
* Writing CDF files:
    - zVariable IBIAS1-IBIAS3, DELTA_PLUS_MINUS are set to fill values/NaN.
    - Some (less important) zVariables might not be set at all (have zero records).
    - Output CDF files use NaN instead of fill/pad values for floating-point values.
        (Fill/pad values for floating-point values are however read correctly from input CDF files.)
    - Global attributes:
