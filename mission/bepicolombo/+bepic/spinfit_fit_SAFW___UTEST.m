%
% matlab.unittest automatic test code for bepic.spinfit.fit_SAFW().
%
%
% Author: Erik P G Johansson, IRF, Uppsala, Sweden
%
classdef spinfit_fit_SAFW___UTEST < matlab.unittest.TestCase
  ...
  methods(Test)
    ...
    function test_all_empty(T)
      actR = bepic.spinfit.fit_SAFW( ...
        ...
        fitWindowCenterRad = pi, ...
        nMinFitSamples     = T.N_MIN_FIT_SAMPLES, ...
        nFitCoefficients   = 5, ...
        dataGapMinNs       = int64(1));

      T.verifyEqual(actR.fitWindowCenterTt2000, int64.empty(0, 1))
    ...
    function test_0(T)
      ...
      T.verifyEqual(actR.fitWindowCenterTt2000, int64([102.5e9]))
      T.verifyEqual(actR.stdDeviation,          [0], AbsTol=1e-14)
    ...
    function test_1(T)
      ...
      T.assertEqual(actR.fitWindowCenterTt2000, int64([101e9; 103e9]))
    ...
    function test_2(T)
      ...
      T.assertEqual(actR.fitWindowCenterTt2000, int64([101e9; 109e9]))
      T.verifyEqual(actR.offset,                      [2; 2], AbsTol=1e-13)
      T.verifyEqual(actR.coefficientCos1,             [3; 3], AbsTol=1e-13)
      T.verifyEqual(actR.coefficientSin1,             [4; 4], AbsTol=1e-13)
      T.verifyEqual(actR.coefficientCos2,             [5; 5], AbsTol=1e-13)
    ...
    % Data gap
    % * Deliberately two fit windows before AND after data gap to test
    %   bepic.spinfit.fit_SAFW()'s merging of processing results.
    ...
    function test_data_gap(T)
      ...
      N = ones(4, 1);
      T.verifyEqual(actR.fitWindowCenterTt2000, int64([101e9; 105e9; 203e9; 207e9]))
      T.verifyEqual(actR.stdDeviation,      0*N, AbsTol=1e-14)
      % Actual<>0*N for unkown reason!!
      % T.verifyEqual(actR.nBadPoints,        [5;5;1;1])
      T.verifyEqual(actR.offset,            2*N, AbsTol=1e-13)
      T.verifyEqual(actR.coefficientCos1,   3*N, AbsTol=1e-13)
      T.verifyEqual(actR.coefficientSin1,   4*N, AbsTol=1e-13)
      T.verifyEqual(actR.coefficientCos2,   5*N, AbsTol=1e-13)
      T.verifyEqual(actR.coefficientSin2,   6*N, AbsTol=1e-13)
    end
