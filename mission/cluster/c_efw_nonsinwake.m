function nonSinWake = c_efw_nonsinwake(cl_id,probe,pha,da)
%C_EFW_NONSINWAKE  detect cold ion wakes by checking for non-sinusoidal
%                  spin variations (often seen in plasmaspheric plumes)
%
% Detects wakes by checking for non-sinusoidal spin variations.
% Routine developed for the paper Andre and Cully, GRL, 2011
% based on the solar wind wake detection/removal routine C_EFW_SWWAKE.
%
% Inputs: cluster ID
%         probe pair (only 12 and 34 implemented)
%         phase (Atwo?)
%         E-field data: c_load(irf_ssub('wE?p!',cl_id,probe));
% Output: affected intervals

nonSinWake=[];
if probe ~= 12 && probe ~= 34
  irf_log('proc','Only probe pairs 12 and 34 can be checked for nonsinusoidal wakes.')
  return
end

% Blank intervals where method can't be used
...
% Check for non-sinusoidal wakes
[da, n_corrected,wake_dsc] = GRL2011_check_wakes(da,probe,pha,whip,0);

% Combine intervals
if n_corrected > 2
  j=1;
  for k=2:length(wake_dsc)-1
    if(wake_dsc(k,1)-wake_dsc(k-1,1)) > 20
      if (wake_dsc(k-1,1)-wake_dsc(j,1)) > 20
        nonSinWake=[nonSinWake wake_dsc(j,1) wake_dsc(k-1,1)+4]; %#ok<AGROW>
      end
      j=k;
    end
  end
  nonSinWake=reshape(nonSinWake,2,length(nonSinWake)/2)';
end

function [data, n_corrected,wakedesc] = GRL2011_check_wakes(e,pair,phase_2,whip,plotflag,extraparams)
% GRL2011_check_wakes
% Check for cold plasma wakes based on non-sinusoidal spins
% Based on C_EFW_SWWAKE.
%
% [data, n_spins_corrected,w_dsc] =
% c_efw_swwake(e,pair,phase_2,[whip,plotflag,extraparams])
%
% Input:
...
for in = iok
  ...
  % Spin in the middle has maximum weight
  ...
  % The proxy wake is narrow (1/2 of the final fit)
  ...
  if max(abs(ccdav1))< WAKE_MIN_AMPLITUDE || ...
      max(abs(ccdav2))< WAKE_MIN_AMPLITUDE || ...
  ...
  elseif ~isempty(cox)
    if DEBUG > 0, irf_log('proc',['start  at ' epoch2iso(ts,1)]), end
  end
  if ~isempty(cox)
    for cx = cox
...
% Clean out single outliers
indx=filter(ones(15,1)/15,1,isfinite(wakedesc(:,1)));
wakedesc(logical(indx < 0.5),:) = NaN;
...
irf_log('proc',['intially corrected ' num2str(n_corrected) ' out of ' ...
  num2str(n_spins) ' spins'])
n_corrected=sum(isfinite(wakedesc(:,1)));
irf_log('proc',['after single point removal: ' num2str(n_corrected) ' out of ' ...
  num2str(n_spins) ' spins'])


function res = c_efw_nonsinwake_isGoodShape(s)
...
maxmax = max(s);
mindepth=min([MINDEPTH*maxmax -0.1]);

d1 = diff(s);
imax = find(((d1(1:end-1).*d1(2:end))<0) & (d1(1:end-1)>0))+1;

if min(s) < mindepth
  irf_log('proc','Signal has a deep minumum')
  res = 0; return
end
...
% eliminate any secondary maxima not separated by a deep enough min
valid=ones(length(imax),1);
for i=2:length(imax)
