...
if isa(r1,'TSeries') && isa(r2,'TSeries') && isa(r3,'TSeries') && isa(r4,'TSeries') && isa(u1,'TSeries') && isa(u2,'TSeries') && isa(u3,'TSeries') && isa(u4,'TSeries')
...
elseif isnumeric(r1) && isnumeric(r2) && isnumeric(r3) && isnumeric(r4) && isnumeric(u1) && isnumeric(u2) && isnumeric(u3) && isnumeric(u4)
...
for i=1:n % to be vectorized at some point
    ...
    dR1=r1i-R_Center;dR2=r2i-R_Center;dR3=r3i-R_Center;dR4=r4i-R_Center;
    
    
    S=0;
    for a=1:4, for b=1:a,     S=S+eval(irf_ssub('(u?i-u!i)''*(dR?-dR!)',a,b));      end,end
    S=S/16;
...
if nargout==0||nargout==1,
    if output_as_TSeries
        gradVec=TSeries(tref,gradVec,'TensorOrder',2,'repres',{'x','y','z'},'repres',{'x','y','z'});
