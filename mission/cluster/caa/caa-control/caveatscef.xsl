...
Copyright (c) 2011-25 Jan Karlsson
...
!--------------------------------------------------------|
FILE_NAME = "C<xsl:value-of select="$clusterno"/>_CQ_EFW_INST__20010128_V03.cef"
...
START_META     =   LOGICAL_FILE_ID
   ENTRY       =   "C<xsl:value-of select="$clusterno"/>_CQ_EFW_INST__20010128_V03"
...
START_META     =   FILE_TIME_SPAN
   ...
   ENTRY       =   2001-01-28T00:00:00Z/2024-10-01T00:00:00Z
...
START_META     =   FILE_CAVEATS
   ENTRY       =   "Cluster <xsl:value-of select="$clusterno"/>. stop time: 2024-10-01T00:00:00Z"
	...
	<xsl:if test="starts-with($clnobool,'yes')">
		...
		<xsl:if test="$dt != -1">
			...
			<!--	<xsl:when test="$dt = -1">2025-01-01T00:00:00Z</xsl:when>        Infinity removed -->
				<xsl:when test="$dt = -157">2024-10-01T00:00:00Z</xsl:when> <!-- end of mission  -->
