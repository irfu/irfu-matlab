...
if 1 % parameters
  ...
  ADA_probe=lp.default_lprobe('PO_ADA');
  ADA_C_antenna=ADA_probe.capacitance; % antenna capacitance in F
  title_text=[title_text 'ADAC_{ant}=' num2str(ADA_probe.capacitance/1e-12) ' pF, '];
  %A_antenna=ADA_probe.Area.total; % antenna area in m2
  title_text=[title_text 'ADAA_{ant}=' num2str(ADA_probe.Area.total,'%5.2f') ' m^2, '];

  % SDP
  SDP_antenna_eff_length=5.5; % efficient distance between antennas
  SDP_R_plasma_nobias=49e6;SDP_R_plasma_bias=40e6; SDP_R_plasma_bias_low=25e6;
  SDP_probe=lp.default_lprobe('PO_SDP');
...
if 1 % read Sahraoui et al. 2009 spectra
  %file reading
  [xx,yy]=textscan('sahraoui2009_fig4.dat','%s%s','headerlines',3);
  for j=1:size(xx,1)
...
if 1 % instrument noise calculations ADA
  if 1 % preamp parameters
    ADA_preamp_noise=150e-9; % preamplifier noise 4nV/Hz1/2
    ADA_preamp_noise_level=(ADA_preamp_noise/ADA_antenna_eff_length)^2;
    f_break=300; % transition frequency at which 1/f noise is starting
  end
  if 1 % preamplifier noise
    ADA_instr_noise=[f(1) f_break f(end)]';
    ADA_instr_noise(:,2)=2*ADA_preamp_noise_level*[(f_break/f(1))^2; 1;  1];
  end
  if 1 % photoelectron thermal noise  S=4kTZ
    T_eV=1; % photoelectron temperature
    ADA_thermal_noise_bias=[f 4*Units.e*T_eV*sqrt(ADA_R_plasma_bias^2./(1+(2*pi*f).^2*ADA_R_plasma_bias^2*ADA_C_antenna^2))/ADA_antenna_eff_length^2];
    ADA_thermal_noise_nobias=[f 4*Units.e*T_eV*sqrt(ADA_R_plasma_nobias^2./(1+(2*pi*f).^2*ADA_R_plasma_nobias^2*ADA_C_antenna^2))/ADA_antenna_eff_length^2];
  end
  if 1 % shot noise plasma and photoelectrons
    ...
    ADA_I=lp.current(ADA_probe,-1,Rsolo,UV,[]);

    ADA_shot_noise_photoelectron_bias=[f 2*Units.e*abs(ADA_I.photo)*(ADA_R_plasma_bias^2./(1+(2*pi*f).^2*ADA_R_plasma_bias^2*ADA_C_antenna^2))/ADA_antenna_eff_length^2];
    ADA_shot_noise_photoelectron_nobias=[f 2*Units.e*abs(ADA_I.photo)*(ADA_R_plasma_nobias^2./(1+(2*pi*f).^2*ADA_R_plasma_nobias^2*ADA_C_antenna^2))/ADA_antenna_eff_length^2];
  end
  if 1 % total noise = shot noise photo + thermal
    ADA_total_noise_bias=irf_add(1,ADA_shot_noise_photoelectron_bias,1,ADA_thermal_noise_bias);
    ADA_total_noise_nobias=irf_add(1,ADA_shot_noise_photoelectron_nobias,1,ADA_thermal_noise_nobias);
  end
  if 1 % bit noise
...
if 1 % EMC requirements
  ADA_EMC.f =      [10    1e2   1e3   1e4   1e5   ];
  %ADA_EMC.Epower = [3*10^-11,3*10^-11,2*10^-11,10^-12,3*10^-13]; % (V/m)^2/Hz
  ADA_EMC.Epower = [2*10^-11,3*10^-12,3*10^-12,3*10^-13, 3*10^-14];

  SDP_EMC.f =      [10      ,1e2     ,1e3     ,1e4   ,1e5];
  %SDP_EMC.Epower = [2*10^-12,10^-14  ,10^-15  ,10^-16,10^-16];
  SDP_EMC.Epower = [3*10^-12,3*10^-12,2*10^-12,10^-13, 10^-14];
end

if 1 % initialize figure - ADA
    hf=figure(12);clf
...
if 1 % electric field plot
  ...
  %title(hca,'Expected electric field spectra and ADA noise levels')
  title(hca,'Expected ADA noise levels')
end

if 1 % plot electric field noises

  if 1 % ADA
    ADA_color = [0.8 0.5 0.5];
    loglog(ADA_total_noise_bias,ADA_total_noise_bias(:,2),'color',ADA_color);hold(hca,'on');
    loglog(ADA_total_noise_nobias(:,1),ADA_total_noise_nobias(:,2),'color',ADA_color,'linestyle',':');
    text(ADA_total_noise_nobias(1,1)*200,ADA_total_noise_nobias(1,2),'ADA nobias noise','fontsize',10,'color',ADA_color,'units','data','horizontalalignment','left','verticalalignment','bottom');
    text(ADA_total_noise_bias(1,1)*200,ADA_total_noise_bias(1,2)*1.2,' plasma+photoelectron\newline fluctuation level','fontsize',10,'color',ADA_color,'units','data','horizontalalignment','left','verticalalignment','bottom');
    loglog(1*[1 5],5e-17*10*[1 1],'color',ADA_color,'linestyle',':');
    loglog(1*[1 5],2e-17*10*[1 1],'color',ADA_color,'linestyle','-');
    text(10.1,8e-16,['unbiased ADA, R=' num2str(ADA_R_plasma_nobias/1e6,3) 'M\Omega'],'horizontalalignment','left','verticalalignment','middle','color','k');
    text(10.1,4e-16,['biased ADA, R=' num2str(ADA_R_plasma_bias/1e6,3) 'M\Omega'],'horizontalalignment','left','verticalalignment','middle','color','k');
    loglog(ADA_instr_noise(:,1), ADA_instr_noise(:,2),'color',[0.3 0.3 0.3]); hold(hca,'on');
    text(ADA_instr_noise(2,1),ADA_instr_noise(end,2)*0.9,'preamp noise','fontsize',10,'color','k','units','data','horizontalalignment','left','verticalalignment','top');
    loglog(hca,ADA_EMC.f,ADA_EMC.Epower,'o-','markersize',10,'color',[0 0.8 0]);
...
irf_legend(0,['PO EFI-ADA noise ' char(datetime("now","Format","uuuu-MM-dd HH:mm:ss"))],[0,0.001],'interpreter','none','color',[0.5 0.5 0.5])
irf_print_fig(['ADA_noise_' char(datetime("now","Format","uuuuMMdd"))],'png')
...
irf_legend(0,['PO EFI-SDP noise ' char(datetime("now","Format","uuuu-MM-dd HH:mm:ss"))],[0,0.001],'interpreter','none','color',[0.5 0.5 0.5])
irf_print_fig(['SDP_noise_' char(datetime("now","Format","uuuuMMdd"))],'png')


%% ADA in dB uV/m
if 1
  EMCR = ADA_EMC; ant = 'ADA';
...
semilogx(ADA_EMC.ff,10*log10(1e6*ADA_EMC.Efield),'m'), hold on
...
SDP_Scale = (50/1)^3; ADA_Scale = (5/1)^3;

loglog(ADA_EMC.ff,sqrt(ADA_Scale^2*ADA_EMC.EEpower),'m'), hold on
