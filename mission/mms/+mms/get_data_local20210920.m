...
vars = {'R_gse','R_gsm','V_gse','V_gsm',...
  ...
  'Omnifluxion_epd_feeps_srvy_l2', 'Omnifluxelectron_epd_feeps_srvy_l2', ...
  'Omnifluxion_epd_eis_brst_l2', 'Omnifluxion_epd_eis_srvy_l2',...
  'PADoxygen_epd_eis_brst_l2','PADoxygen_epd_eis_brst_l2'}; % XXX THESE MUST BE THE SAME VARS AS BELOW
...
switch Vr.inst
  ...
  case 'edi'
    switch Vr.param
      ...
      case {'Counts-amb'}
        ...
        nodes = 1:4;
        gdu = [1 2];
        flux = cell(numel(gdu),numel(nodes));
        %mmsIdS
        for igdu = 1:numel(gdu)
          for inode = 1:numel(nodes)            
            pref = ['mms' mmsIdS '_' Vr.inst '_amb_gdu' num2str(igdu)  '_raw_counts' num2str(inode)];
            flux{igdu,inode} = get_ts('scalar');            
          end
        end
        if isempty(flux{1,1})
          res = [];
        ...
        res.siConversion = flux{1}.siConversion;
        res.species = 'electrons';
        tmp_name = pref; tmp_name(3)='*'; tmp_name = strrep(tmp_name,'180','*'); tmp_name = strrep(tmp_name,'0','*');
        res.name = tmp_name;
        % ancillary data
        res.ancillary.dt_minus = 0.5*(flux{1}.time(2)-flux{1}.time(1));
        res.ancillary.dt_plus = 0.5*(flux{1}.time(2)-flux{1}.time(1));
  ...
  case 'epd_eis'
    all_species = {'ion','electron','oxygen'};
    species_index = cellfun(@(s) ~isempty(strfind(Vr.param, s)), all_species);
    species = all_species{find(species_index)};    
    param = extractBefore(Vr.param,species);
    switch Vr.param
      case {'PADoxygen'}
        mes = 'extof'; % Energy x Time-of-flight
      otherwise
        mes = 'phxtof'; % Energy x Pulse hight
    end
    dsetName = ['mms' mmsIdS '_' strrep(Vr.inst,'_','-') '_' Vr.tmmode '_' Vr.lev '_' mes];
   % mms?_epd-eis_srvy_l2_phxtof
   switch Vr.param
      case {'PADoxygen'}
        res = get_ts('eis_pad');
      otherwise
        res = get_ts('eis_omni'); % old implementation
    end
    %res = get_ts('eis_omni');
  otherwise
    error('not implemented yet')
  ...
  function res = get_ts(dataType)
    ...
    switch dataType
      ...
      case {'eis_pad'}
        ...
        % This is first data object... Need to generalize to many data
        % files...
        dobj = dataobj([file_list(1).path '/' file_list(1).name]);
        time = get_variable(dobj,'Epoch'); time = EpochTT(time.data);
        dtminus = get_variable(dobj,'Time_Minus'); dtminus = EpochTT(dtminus.data);
        dtplus = get_variable(dobj,'Time_Minus'); dtplus = EpochTT(dtplus.data);
                
        % Get pitch angle information from each detector
        pitch_angle_all = nan(time.length,6);
        for iSen = 0:5
          switch Vr.tmmode
          	case 'brst'                    
            	varname = ['mms' mmsIdS '_epd_eis_' Vr.tmmode '_' mes '_pitch_angle_t' num2str(iSen)];
            %case 'srvy'
            	%    pref = ['mms' mmsIdS '_epd_eis_phxtof_proton_P4_flux_t' num2str(iSen)];
            otherwise, error('invalid mode')
            end
          pa_tmp = get_variable(dobj,varname); 
          pitch_angle_all(:,iSen+1) = pa_tmp.data;
        end
        % Get energy information from each detector
        % Same for all times
        % Also same for all sensors, but they list them separately anyways
        for iSen = 0:5
          switch Vr.tmmode            
          	case 'brst'   
%               mms2_epd_eis_brst_extof_oxygen_t3_energy : double : 1 recs
%               mms2_epd_eis_brst_extof_oxygen_t3_energy_dminus : double : 1 recs
%               mms2_epd_eis_brst_extof_oxygen_t3_energy_dplus : double : 1 recs
            	varname = ['mms' mmsIdS '_epd_eis_' Vr.tmmode '_' mes '_' species '_t' num2str(iSen) '_energy'];
            %case 'srvy'
            	%    pref = ['mms' mmsIdS '_epd_eis_phxtof_proton_P4_flux_t' num2str(iSen)];
            otherwise, error('invalid mode')
          end
            if 0
          energy_tmp = get_variable(dobj,varname); 
          energy_dminus_tmp = get_variable(dobj,[varname '_dminus']); 
          energy_dplus_tmp = get_variable(dobj,[varname '_dplus']); 
          energy_all(:,iSen+1) = energy_tmp.data;
          energy_dminus_all(:,iSen+1) = energy_dminus_tmp.data;
          energy_dplus_all(:,iSen+1) = energy_dplus_tmp.data;
          nEnergy = numel(energy_tmp.data);
            else              
%           energy_tmp = get_variable(dobj,varname); 
%           energy_dminus_tmp = get_variable(dobj,[varname '_dminus']); 
%           energy_dplus_tmp = get_variable(dobj,[varname '_dplus']); 
          energy_all(:,iSen+1) = dobj.data.(varname).data;
          energy_dminus_all(:,iSen+1) = dobj.data.([varname '_dminus']).data;
          energy_dplus_all(:,iSen+1) =dobj.data.(varname).data;
            end
        end
        % Get data
        for iSen = 0:5
          ...
          tmpvar = mms.db_get_ts(dsetName,pref,Tint);
          if not(isempty(tmpvar))
            EIScounts{iSen+1} = comb_ts(tmpvar);
          end
        end
        % check if energies are equal or not.
        for iSen = 0:4
...
switch param
  case {'Ni', 'partNi', 'Ne', 'partNe', 'Nhplus', 'Tsi', 'Tperpi', 'Tparai', 'partTperpi', 'partTparai', ...
      ...
      'Flux-amb-pm2','Flux-err-amb-pm2','Counts-amb'}
    tensorOrder = 0;
...
if length(tk)==idx, dataLevel = 'l2'; %default
  ...
  dataLevel = tk{idx+1};
  switch dataLevel
    case {'ql','sitl','l1b','l2a','l2pre','l2','l3','l1a'}
    otherwise
