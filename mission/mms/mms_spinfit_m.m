...
%   timeData - time of measurement (1D array, int64, TT2000)
%   data     - data (samples) to be fitted
%   phase    - phase of instrument at corresponding time of measurement
%              (1D array, radians)
%   fitEvery - time between every spin fit time interval (ns)
%              (default 5*10^9 ns)
%   fitInter - length of time window used for each separate spin fit
%              (default 20*10^9 ns)
%   t0       - the first time inside timeData which is evenly divisible
...
%
% Output: (all required)
%   timeFit  - middle of each spinfit (00:00:05, 00:00:10 etc).
%              (1D array, int64 TT2000)
%   sfit     - matrix with each fit coefficents
%   sdev     - standard deviation of each fit (1D array)
%   iter     - number of iterations used for each fit (1D array)
%   nBad     - number of bad points, outliers for each fit (1D array)
%
