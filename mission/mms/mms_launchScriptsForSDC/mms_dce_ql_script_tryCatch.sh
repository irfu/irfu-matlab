...
# Run if two input files (DCE and sunpulse)
   matlab -nodesktop -nosplash -nodisplay -nojvm -r "try, mms_sdc_sdp_proc('ql','$1','$2'), catch err, if(strcmp(err.identifier,'MATLAB:SDCcode')) irf.log('critical',['Bash error catch worked: ', err.identifier, '. With message: ', err.message]); exit(str2num(err.message)); else irf.log('critical',['Bash error catch: ', err.identifier, '. With message: ', err.message]); exit(199); end; end, exit(0)"
elif [ ${#} -eq 3 ] ; then
# RUN THIS IF three FILES EXISTS (DCE and DCV and sunpulse)
   matlab -nodesktop -nosplash -nodisplay -nojvm -r "try, mms_sdc_sdp_proc('ql','$1','$2','$3'), catch err, if(strcmp(err.identifier,'MATLAB:SDCcode')) irf.log('critical',['Bash error catch worked: ', err.identifier, '. With message: ', err.message]); exit(str2num(err.message)); else irf.log('critical',['Bash error catch: ', err.identifier, '. With message: ', err.message]); exit(199); end; end, exit(0)"
fi
