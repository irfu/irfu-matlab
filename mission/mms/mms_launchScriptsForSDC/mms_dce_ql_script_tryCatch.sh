...
#    <mmsX_***_dce_filename.cdf> = Filename of DC E data to be processed for QL. Including path and extension.
#    <mmsX_***_dcv_filename.cdf> = Filename of DC V data to be processed for QL. Including path and extention.
#    <mmsX_***_101_filename.cdf> = Filename of HK 101 data (with sunpulse) to be processed for QL. Including path and extention.
#  output files created:
#    <mmsX_***_ql_dce2d_yyyymmddHHMMSS_vX.Y.Z.cdf>         = File placed in $DROPBOX_ROOT
#    <DATE_IRFU.log>                                       = Logfile of run, placed in $LOG_PATH_ROOT/mmsX/sdp/.
#    <mmsX_***_ql_dce2d_yyyymmddHHMMSS_vX.Y.Z_runTime.log> = File to identify output file, as per e-mail of 2013/11/22. Also placed in $LOG_PATH_ROOT/mmsX/sdp/.
#
#  return code 0 if ok.
...
# Run if two input files (DCE and sunpulse)
   /tools/matlab/R2013b/bin/matlab -nodesktop -nosplash -nodisplay -nojvm -r "try, mms_sdc_sdp_proc('ql','$1','$2'), catch err, if(strcmp(err.identifier,'MATLAB:SDCcode')) irf.log('critical',['Bash error catch worked: ', err.identifier, '. With message: ', err.message]); exit(str2num(err.message)); else irf.log('critical',['Bash error catch: ', err.identifier, '. With message: ', err.message]); exit(199); end; end, exit(0)"
elif [ ${#} -eq 3 ] ; then
# RUN THIS IF three FILES EXISTS (DCE and DCV and sunpulse)
   /tools/matlab/R2013b/bin/matlab -nodesktop -nosplash -nodisplay -nojvm -r "try, mms_sdc_sdp_proc('ql','$1','$2','$3'), catch err, if(strcmp(err.identifier,'MATLAB:SDCcode')) irf.log('critical',['Bash error catch worked: ', err.identifier, '. With message: ', err.message]); exit(str2num(err.message)); else irf.log('critical',['Bash error catch: ', err.identifier, '. With message: ', err.message]); exit(199); end; end, exit(0)"
fi
