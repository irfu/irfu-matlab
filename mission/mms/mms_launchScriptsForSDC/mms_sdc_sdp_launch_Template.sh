...
#$MATLAB_EXE $MATLAB_FLAGS -r "try, mms_sdc_sdp_proc('$PROCESS_NAME','$1','$2','$3','$4'), catch ME, logFile=irf.log('log_out'); if(exist(logFile,'file')), unix(['echo ''''Error occured at SDC! Please see the attached log file. Causing error was: ',ME.identifier,' with message: ',ME.message,''''' | mail --encoding=8BIT --attach=', logFile,' -s MMS_SDC_Error thoni@localhost']); else unix(['echo ''''Error occured at SDC. And for some reason no log file was identified, manually log into SDC and have a look! Causing error was: ',ME.identifier,' with message: ',ME.message,''''' | mail -s MMS_SDC_Error thoni@localhost']); end; exit(199); end, exit(0)"
