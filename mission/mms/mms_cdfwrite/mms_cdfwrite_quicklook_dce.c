/*
Filename: irfu_cdfwrite_quicklook_dce.c 
...
//        prhs[6] - quality / errors, variable of epoch. ?

if(nrhs!=7)
{
  mexErrMsgIdAndTxt("MATLAB:irfu_cdfwrite:inputarg",
     "Seven input arguments required. 'Filename', 'SC_id', 'Epoch', 'mmsX_sdp_dce_xyz_pgse', 'mmsX_spd_dce_xyz_dsl', 'bitmask', 'quality'"); 
}
...
/////
// Eight (actually sixth) write variable attributes to mmsX_sdp_dce_quality_variable (named spd_dce_quality)
/////
...
status = CDFputzVarAllRecordsByVarID (id, QUALITYvarNum, mxGetM(prhs[6]), (uint32_t *)mxGetData(prhs[6]));
  if (status != CDF_OK) UserStatusHandler (status);
