...
for scId=1:4
  try
    ...
    if(~isempty(list))
      ...
      if any(ind)
        warning('Found %i rows with time not being monotonically increasing.', length(ind));
      ...
      if any(ind)
        ind = unique(sort(ind));
        warning('Found %i rows with NaN in offset file.', length(ind));
        warning(['First (max 5) rows with problems were: ', sprintf('%i, ', ind(1:min(5, length(ind))))]);
        calOk = false;
      end
