...
switch procId
  case {MMS_CONST.SDCProc.sitl, MMS_CONST.SDCProc.ql, MMS_CONST.SDCProc.l2e}
    ...
    GATTRIB.Logical_source_description = {sprintf(...
      'MMS %i spin-plane dual probe %s (%s), two dimensional electric field.',...
      scId,procName,tmModeStr)}; % in full words.
  ...
  case MMS_CONST.SDCProc.usc
    ...
    name.epoch   = sprintf('mms%i_%s_epoch_dcv',scId,INST_NAME); % Timestamp in TT2000
    name.escp    = sprintf('mms%i_%s_escp_dcv',scId,INST_NAME); % Estimated Spacecraft potential
    name.psp     = sprintf('mms%i_%s_psp_dcv',scId,INST_NAME); % Probe to spacecraft potential
    name.delta   = sprintf('mms%i_%s_delta_dcv',scId,INST_NAME); % Delta
    name.psp_p   = sprintf('mms%i_%s_psp_probes_dcv',scId,INST_NAME); % Probe to spacecraft potential, indiv probes
    name.bitmask = sprintf('mms%i_%s_bitmask_dcv',scId,INST_NAME); % Bitmask
    name.label   = 'LABL_1';
    ...
    GATTRIB.Logical_source = {sprintf('mms%i_%s_%s_%s_l2_uscdcv',scId,INST_NAME,tmModeStr)}; % ie. mms2_sdp_fast_l2_uscdcv
