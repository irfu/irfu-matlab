            ...
            function epoch = get_time(s)
              ...
              try
                ...
                try
                  ...
                  % char.
                  regStr = regexp(out, ...
                    '(?<YYYY>\d{4})-(?<DOY>\d{3}).(?<HH>\d{2}):(?<MM>\d{2}):(?<SS>\d{2}).(?<ms>\d{1,3})', ...
                    'names');
                  doy8 = [str2double(regStr.YYYY), str2double(regStr.DOY), ...
                    str2double(regStr.HH), str2double(regStr.MM), ...
                    str2double(regStr.SS), str2double(regStr.ms), 0, 0];
                  sss = irf_time(doy8, 'doy8>ttns');
                  epoch = EpochTT(sss);
                catch ME2
