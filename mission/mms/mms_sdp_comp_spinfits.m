    ...
    if( (dce.time(1)<=t0) && (t0<=dce.time(end)))
      for iPair=1:numel(sdpPair)
        ...
        [time, sfit.(sdpPair{iPair}), sdev.(sdpPair{iPair}), ...
          ...
          mms_spinfit_mx(maxIt, minPts, nTerms, ...
          double( dce.time( ~bitand(dce.(sdpPair{iPair}).bitmask,MMS_CONST.Bitmask.SIGNAL_OFF) ) )', ...
          double( dce.(sdpPair{iPair}).data( ~bitand(dce.(sdpPair{iPair}).bitmask,MMS_CONST.Bitmask.SIGNAL_OFF) ) )', ...
          phase.data( ~bitand(dce.(sdpPair{iPair}).bitmask,MMS_CONST.Bitmask.SIGNAL_OFF) )' + (180/pi)*MMS_CONST.Phaseshift.(sdpPair{iPair}) ,...
