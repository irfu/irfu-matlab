...
switch procId
  case MMS_CONST.SDCProc.ql
    ...
    dminus  = dplus; % Symmetric sampling.
    repres1 = ['x'; 'y'; 'z'];
    ...
    name.dplus   = [datasetPrefix '_deltaplus']; % Delta Plus (time interval)
    name.dminus  = [datasetPrefix '_deltaminus']; % Delta Minus (time interval)
    ...
    outVars = {name.epoch, epochTT, name.label, label, name.dsl, dsl, ...
      ...
      name.dplus, dplus, name.dminus, dminus, name.repres1, repres1};
    ...
    varDatatype = {name.epoch, getfield(mms_sdp_typecast('epoch'),'cdf'), ...
      ...
      name.dminus,  getfield(mms_sdp_typecast('deltaplus'),'cdf'), ...
    ...
    compressVars = {name.label, COMPRESS_LEVEL, name.dsl, COMPRESS_LEVEL, ...
      ...
      name.dplus, COMPRESS_LEVEL, name.dminus, COMPRESS_LEVEL, ...
      name.repres1, COMPRESS_LEVEL};
    ...
    VATTRIB.CATDESC = {name.epoch, 'Time tags, in TT2000'; ...
      ...
      name.dplus,   'Delta plus, time interval during which the data was measured'; ...
      name.dminus,  'Delta minus, time inteval during which the data was measured'; ...
      name.repres1, 'Representation'};
    VATTRIB.COORDINATE_SYSTEM = {name.dsl, 'DSL'};
    VATTRIB.DELTA_PLUS_VAR = {name.epoch, name.dplus};
    VATTRIB.DELTA_MINUS_VAR = {name.epoch, name.dminus};
    ...
    VATTRIB.FIELDNAM = {name.epoch, 'Time tags'; ...
      ...
      name.dplus,   'Delta plus';...
      name.dminus,  'Delta minus';...
      name.repres1, 'Representation'};
    VATTRIB.FILLVAL = {name.epoch, getfield(mms_sdp_typecast('epoch'),'fillval'); ...
      ...
      name.dminus,  getfield(mms_sdp_typecast('deltaplus'), 'fillval')};
    ...
    VATTRIB.REPRES_PTR_1 = {name.dsl, name.repres1};
    VATTRIB.SI_CONVERSION = {name.dsl,     '1.0e-3>V m^-1'; ...
      name.dplus,   '1.0e-9>s';...
      name.dminus,  '1.0e-9>s'};
    ...
    VATTRIB.VALIDMIN = {name.epoch, spdfcomputett2000([1990,01,01,0,0,0,0,0,0]); ...
      ...
      name.dminus,  mms_sdp_typecast('deltaplus',61000)};
    VATTRIB.VALIDMAX = {name.epoch, spdfcomputett2000([2100,01,01,0,0,0,0,0,0]); ...
      ...
      name.dminus,  mms_sdp_typecast('deltaplus',62500000)};
  ...
  case MMS_CONST.SDCProc.l2a
    ...
    dminus2  = dplus2; % Symmetric sampling.
    ...
    name.dplus   = [datasetPrefix '_deltaplus_' varNameSuffix]; % Delta Plus (time interval)
    name.dminus  = [datasetPrefix '_deltaminus_' varNameSuffix]; % Delta Minus (time interval)
    name.dplus2  = [datasetPrefix '_deltaplus2_' varNameSuffix]; % Delta Plus (time interval for spinfits)
    name.dminus2 = [datasetPrefix '_deltaminus2_' varNameSuffix]; % Delta Minus (time interval for spinfits)
    name.sfitSdev = [datasetPrefix '_sdev_spinfit_' varNameSuffix]; % Spinfit SDEV.
    name.sfitAfit = [datasetPrefix '_a_spinfit_' varNameSuffix]; % Spinfit A coeff.
    ...
    outVars = {name.epoch, epochTT, name.label, label, name.dce, dcedata, ...
      ...
      name.dplus, dplus, name.dminus, dminus, ...
      name.dplus2, dplus2, name.dminus2, dminus2};
    % RecordBound, ie individual cdf records for each row.
    recBound = {name.epoch, name.dce, name.phase, name.bitmask, ...
      name.quality, name.adc, name.sfitsEpoch, name.sfitSdev, name.sfitAfit};
    %Variable Datatypes
    varDatatype = {name.epoch, getfield(mms_sdp_typecast('epoch'),'cdf'), ...
      name.label,      getfield(mms_sdp_typecast('label'),     'cdf'), ...
      ...
      name.dminus2,    getfield(mms_sdp_typecast('deltaplus'),  'cdf'), ...
      name.sfitSdev,   getfield(mms_sdp_typecast('spinfits'),   'cdf'), ...
      name.sfitAfit,   getfield(mms_sdp_typecast('spinfits'),   'cdf')};
    % Compression level for each variable. Note: Not for TT2000 epoch.
    compressVars = {name.dce, COMPRESS_LEVEL,...
      ...
      name.dminus, COMPRESS_LEVEL, name.dplus2, COMPRESS_LEVEL, ...
      name.dminus2, COMPRESS_LEVEL, name.sfitSdev, COMPRESS_LEVEL, ...
      name.sfitAfit, COMPRESS_LEVEL};
    ...
    VATTRIB.CATDESC = {name.epoch, 'Time tags, in TT2000'; ...
      ...
      name.dplus,      'Delta plus, time interval during which the data was measured'; ...
      name.dminus,     'Delta minus, time inteval during which the data was measured';...
      name.dplus2,     'Delta plus, time interval during which the data was measured'; ...
      name.dminus2,    'Delta minus, time inteval during which the data was measured'; ...
      name.sfitSdev,   'Standard deviation of spinfits'; ...
      name.sfitAfit,   'A coeff. (dc level) of spinfits'};
    VATTRIB.COORDINATE_SYSTEM = {name.dce, 'BCS'};
    VATTRIB.DELTA_PLUS_VAR = {name.epoch, name.dplus; ...
      name.sfitsEpoch, name.dplus2};
    VATTRIB.DELTA_MINUS_VAR = {name.epoch, name.dminus; ...
      name.sfitsEpoch, name.dminus2};
    ...
    VATTRIB.FIELDNAM = {name.epoch, 'Time tags'; ...
      ...
      name.dplus,      'Delta plus';...
      name.dminus,     'Delta minus';...
      name.dplus2,     'Delta plus';...
      name.dminus2,    'Delta minus'; ...
      name.sfitSdev,   'Sdev spinfits'; ...
      name.sfitAfit,   'A coeff. spinfits'};
    VATTRIB.FILLVAL = {name.epoch, getfield(mms_sdp_typecast('epoch'),'fillval'); ...
      ...
      name.dminus2,    getfield(mms_sdp_typecast('deltaplus'), 'fillval'); ...
    ...
    VATTRIB.LABL_PTR_1 = {name.dce, name.label; name.bitmask, name.label; ...
      name.adc, name.label3; name.delta, name.label4; ...
      name.sfitSdev, name.label3; name.sfitAfit, name.label3};
    ...
    VATTRIB.TENSOR_ORDER = {name.dce, mms_sdp_typecast('tensor_order',1);...
      ...
      name.quality, mms_sdp_typecast('tensor_order',0);...
      name.bitmask, mms_sdp_typecast('tensor_order',0);...
      name.delta,   mms_sdp_typecast('tensor_order',0);...
    ...
    VATTRIB.VALIDMIN = {name.epoch, spdfcomputett2000([1990,01,01,0,0,0,0,0,0]); ...
      ...
      name.dminus2, mms_sdp_typecast('deltaplus',0); ...
    ...
    VATTRIB.VALIDMAX = {name.epoch, spdfcomputett2000([2100,01,01,0,0,0,0,0,0]); ...
      ...
      name.dminus2,    mms_sdp_typecast('deltaplus',1000000000000); ...
      name.sfitSdev,   EFIELD_MAX; ...
      name.sfitAfit,   EFIELD_MAX};
    VATTRIB.VAR_TYPE = {name.epoch, 'support_data'; ...
      name.label,      'metadata'; ...
    ...
    sdev = zeros(length(spinfit.time),numel(sdpPair));
    afit = zeros(length(spinfit.time),numel(sdpPair));

    for iPair=1:numel(sdpPair)
      name.sfits.(sdpPair{iPair}) = [datasetPrefix,'_', sdpPair{iPair}, '_spinfit_', varNameSuffix];
      % Update output variables
      outVars = [outVars {...
        name.sfits.(sdpPair{iPair}), spinfit.sfit.(sdpPair{iPair})(:,2:end)}]; %#ok<AGROW> %Sfit coeff. B&C and above
      % Separate out SDEV and AFIT all probe pairs
      sdev(:,iPair) = spinfit.sdev.(sdpPair{iPair}); % Standard deviation
      afit(:,iPair) = spinfit.sfit.(sdpPair{iPair})(:,1); % Sfit coeff. A.
      recBound = [recBound {name.sfits.(sdpPair{iPair})}]; %#ok<AGROW>
      varDatatype = [varDatatype {...
        name.sfits.(sdpPair{iPair}), getfield(mms_sdp_typecast('spinfits'),'cdf')}]; %#ok<AGROW>
      compressVars = [compressVars {...
        name.sfits.(sdpPair{iPair}), COMPRESS_LEVEL}]; %#ok<AGROW>
      % Update VATTRIB
      VATTRIB.CATDESC = [VATTRIB.CATDESC; {...
        name.sfits.(sdpPair{iPair}), ['Spinfit: coeffs B,C and up of y=A+B*cos(wt)+C*sin(wt)+... ', sdpPair{iPair}]}];
    ...
    % Now add the extracted SDEV and AFIT to outVars
    outVars = [outVars {name.sfitSdev, mms_sdp_typecast('spinfits',sdev), ...
      name.sfitAfit, mms_sdp_typecast('spinfits',afit)}];
  ...
  case MMS_CONST.SDCProc.l2pre
    ...
    deltadata = mms_sdp_typecast('delta_offset',[real(l2pre.delta_off), imag(l2pre.delta_off)]);
    ...
    name.dplus   = [datasetPrefix '_deltaplus_' varNameSuffix]; % Delta Plus (time interval)
    name.dminus  = [datasetPrefix '_deltaminus_' varNameSuffix]; % Delta Minus (time interval)
    name.repres1 = [datasetPrefix '_representation_' varNameSuffix];
    label        = ['DSL_X'; 'DSL_Y'; 'DSL_Z'];
    label2       = ['E_12'; 'E_34'];
    ...
    recBound = {name.epoch, name.dsl, name.phase, name.bitmask, ...
      name.quality, name.adp, name.adc};
    %Variable Datatypes
    varDatatype = {name.epoch, getfield(mms_sdp_typecast('epoch'),'cdf'), ...
      name.label,   getfield(mms_sdp_typecast('label'),'cdf'), ...
      name.label2,  getfield(mms_sdp_typecast('label'),'cdf'), ...
    ...
    VATTRIB.CATDESC = {name.epoch, 'Time tags, in TT2000'; ...
      ...
      name.delta,   'Delta offset which has been removed between probe pair E12 & E34.';...
      name.dplus,   'Delta plus, time interval during which the data was measured'; ...
      name.dminus,  'Delta minus, time inteval during which the data was measured'; ...
      name.repres1, 'Representation'};
    ...
    VATTRIB.FORMAT = {name.label,   'A23'; ...
      ...
      name.dminus,  'I19'; ...
      name.repres1, 'A23'};
    VATTRIB.LABLAXIS = {name.adp, 'ADP measurement'};
    VATTRIB.LABL_PTR_1 = {name.dsl, name.label;...
    ...
    VATTRIB.SI_CONVERSION = {name.dsl,     '1.0e-3>V m^-1'; ...
      ...
      name.phase,   '0.0174532925>rad';...
      name.adc,     '1.0e-3V m^-1';...
      name.delta,   '1.0e-3V m^-1'; ...
      name.dplus,   '1.0e-9>s';...
      name.dminus,  '1.0e-9>s'};
    VATTRIB.TENSOR_ORDER = {name.dsl, mms_sdp_typecast('tensor_order',1); ...
      name.phase,   mms_sdp_typecast('tensor_order',0); ...
...
%    GATTRIB.Parents = cell(0,1); % Req if number of source cdf >= 2.
  ...
  function VATTRIB = getVariableAttributes
    ...
    VATTRIB.REPRESENTATION_1 = cell(0,1); % Strongly recommended for vector/tensors/quaternions. (one dimension)
    VATTRIB.REPRES_PTR_1 = cell(0,1);     % Strongly recommended for vector/tensors/quaternions, (multiple dimensions similar to LABL_PTR_1) 
