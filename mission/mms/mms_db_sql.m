...
keyboard; % THIS FUNCTION IS NOT FULLY TESTED, MAKE SURE TO MAKE A BACKUP OF THE DB FILE
    ...
    function add_all_files_to_import_list(obj)
      ...
      system(['cd ' obj.databaseDirectory ...
        ...
        'perl -pe ''s/(\.\/mms.*)(mms[1-4]?_[\w-]*)_(20\d\d\d\d\d\d\d*)_v(\d{1,4}).(\d{1,4}).(\d{1,4})(.cdf)\n/$1,$2,$3,$4,$5,$6,$_/'' > delme.txt;' ...
      ...
      system(['cd ' obj.databaseDirectory ...
        ...
        'echo -e ".mod csv\n.import delme.txt FileListToImport\n" | sqlite3 ' obj.databaseFile ';'...
        'rm ./delme.txt' ]);
      irf.log('notice','Clearing up superseeded files inside FileListToImport.');
      sql = ['DELETE FROM FileListToImport WHERE fileNameFullPath NOT IN ', ...
        '(SELECT t.fileNameFullPath FROM FileListToImport AS t ', ...
        'WHERE NOT EXISTS (SELECT 1 FROM FileListToImport AS t2 ', ...
        'WHERE t.directory = t2.directory AND t.dataset = t2.dataset ', ...
        'AND t.date = t2.date AND ( (t2.verX > t.verX) ', ...
        'OR (t2.verX = t.verX AND t2.verY > t.verY) ', ...
        'OR (t2.verX = t.verX AND t2.verY = t.verY AND t2.verZ > t.verZ) ) ) );'];
      obj.sqlUpdate(sql);
    end
