...
%   ECT_combined (product of Boyd et al 2019, 2021):
%   FEDU, FEDU_FIT, FEDO, FEDO_FIT
%   EMFISIS:
...
switch lower(instr)
    ...
    case 'ect-combined'
        ...
        instrumentID = ['ect', filesep, strrep(instr,'-','_')];
        cdfname = [RBSPID '_ect-elec-' upper(levelID) '_'];
        return_type = 'ect_combined-PA';
...
while true
    ...
    function res = get_ts
        ...
        switch return_type
            ...
            case 'ect_combined-PA'
                ...
                vs = split(VAR,'_');
                if isscalar(vs)
                    ixe = strcmpi(var_names,'FEDU_Energy');
                elseif strcmpi(vs{2},'fit')
                    ixe = strcmpi(var_names,'FEDU_FIT_Energy');
                else
                    ixe = strcmpi(var_names,'FEDU_Energy');
                end
                E = load_cdf{ixe};
                if ~con
                    %pitch angle
                    ixpa = strcmpi(var_names,'FEDU_Alpha');
                    Alpha = load_cdf{ixpa};
                end
                %build the pdist object
                if ~con
                ...
                res.userData.VariableAttribute = load_var.Attributes;
                %add anciellary
                ixtt = strcmpi(var_names,'FEDU_Energy_DELTA_minus');
                res.ancillary.delta_energy_minus = load_cdf{ixtt};
                ixtt = strcmpi(var_names,'FEDU_Energy_DELTA_plus');
                res.ancillary.delta_energy_plus = load_cdf{ixtt};
                if add_anc
                    ixtt = strcmpi(var_names,'L_star');
                    res.ancillary.L_star = load_cdf{ixtt}(ix_tlim);
    ...
    function res = combine_ts(inp1,inp2)
        if isa(inp1,'PDist')
            ...
            %%ancillary. This function works for PDist with type omni and
            %%pitchangle only.
            if ~(strcmpi(inp1.type,'omni') || strcmpi(inp1.type,'pitchangle'))
                error('PDist type not known for combine_ts')
            end
            ...
            if strcmpi(inp1.type,'pitchangle')
                th1 = inp1.depend{2};
                th2 = inp2.depend{2};
            ...
            elseif strcmpi(inp1.type,'omni')
                res = PDist(EpochUnix(Tn),Datn,'omni',En);
