function out = gui_initialize( varargin )
%LP.GUI_INITIALIZE Initialize LP environment
%
...
while ~isempty(args)
	...
	elseif isa(args{1},'lp.plasma')
		PlasmaList = args{1};
		args(1) =[];
	elseif isa(args{1},'matlab.ui.Figure')
		openNewFigure = false;
		figH = args{1};
		args(1) = [];
	end
...
set_ide(figH,SpacecraftList,ProbeList,PlasmaList);
out = figH.UserData.inp;

%lp.sweep_gui('update');

	function figH = new_ide
		%% initialize figure
