  ...
  methods
    ...
    function PD = d3v(obj,varargin)
      ...
      if 0
        ...
        v_centers = sqrt(2*units.e*obj.depend{1}/mass); % m/s
        ...
        vel_mat = repmat(v_centers,1,1,32,16);

        d3v = d_vel_mat.*d_azim.*d_polar_mat; % (m/s)^3
      else
        %%
        % d3v = v^2*dv*sin(pol)*dpol*dazim
        % Polar angles
        centers_polar = obj.depend{3};
        d_polar = mean(diff(centers_polar)); % always assumed to be the same
        edges_polar = [centers_polar - 0.5*d_polar centers_polar(end) + 0.5*d_polar];
        d_p = diff(edges_polar);

        % Azimuthal angles
        centers_azim = obj.depend{2}(1,:); % always assumed to be the same size, just different shifts
        d_azim = mean(diff(centers_azim));
        edges_azim = [centers_azim - 0.5*d_azim centers_azim(end) + 0.5*d_azim];        
        d_azim = diff(edges_azim);
        
        % velocities
        if doScpot
          centers_E = obj.depend{1} - repmat(scpot.data,1,size(obj.depend{1},2));
          centers_E(centers_E<0) = 0;
          E_minus = (obj.depend{1} - obj.ancillary.delta_energy_minus) - repmat(scpot.data,1,size(obj.depend{1},2));
          E_plus = (obj.depend{1} + obj.ancillary.delta_energy_plus)   - repmat(scpot.data,1,size(obj.depend{1},2));
          E_minus(E_minus<0)= 0;
          E_plus(E_plus<0)= 0;
        else
          centers_E = obj.depend{1};
          E_minus = (obj.depend{1} - obj.ancillary.delta_energy_minus);
          E_plus = (obj.depend{1} + obj.ancillary.delta_energy_plus);
        end
        v_centers = sqrt(2*units.e*centers_E/mass); % m/s
        v_minus = sqrt(2*units.e*E_minus/mass); % m/s
        v_plus = sqrt(2*units.e*E_plus/mass); % m/s
        d_v = v_plus - v_minus;
        %VV = repmat(v_center,1,1,32,16);
      
        [VV,AA,PP] = ndgrid(v_centers,centers_azim,centers_polar);
        [dVV,dAA,dPP] = ndgrid(d_v,d_azim,d_p);
        d3v = VV.^2.*dVV.*dAA.*sind(PP).*dPP;
      ...
      if doReturnMat
        ...
        PD = obj;
        PD.data = reshape(d3v,[1 size(d3v)])*d3v_scale;
        PD.units = new_units;
        PD.name = 'd3v';
      ...
      while have_options
        switch(lower(args{1}))
          ...
          case 'lowerelim'
            ...
            ancillary_data{end+1} = lowerelim;
            doLowerElim = 1;
            if isnumeric(lowerelim) && isscalar(lowerelim)
    ...
    function PD = shift_many(pdist,v_nf,nMC,orient,sc,varargin)
      % Same as PDist.shift, but adds an envelope so that one can do a
      % complete time series at a time. PDist.shift only accepts 
      % single-time PDists.
      ...
      for it = 1:nt
        time = times(it);
        PD_old_tmp = pdist.tlim(time + 0.001*[-1 1]);
        PD_new_tmp = PD_old_tmp.shift(v_nf,nMC,orient,sc,varargin{:});
        if it == 1
          %PD = PD_new_tmp;
          % Initialize matrices
          % Main
          data   = zeros([nt,size(squeeze(PD_new_tmp.data))]);
          energy = zeros(nt,numel(PD_new_tmp.depend{1}));
          azim   = zeros(nt,numel(PD_new_tmp.depend{2}));
          polar  = zeros(nt,numel(PD_new_tmp.depend{3}));

          % Ancillary
          V_edges            = zeros(nt,numel(PD_new_tmp.ancillary.V_edges));
          phi_edges          = zeros(nt,numel(PD_new_tmp.ancillary.phi_edges));
          theta_edges        = zeros(nt,numel(PD_new_tmp.ancillary.theta_edges));
          energy0            = zeros(nt,numel(PD_new_tmp.ancillary.energy0));
          energy1            = zeros(nt,numel(PD_new_tmp.ancillary.energy1));
          esteptable         = zeros(nt,numel(PD_new_tmp.ancillary.esteptable));
          % energy            = zeros(nt,numel(PD_new_tmp.ancillary.energy)); % same as above
          delta_energy_plus  = zeros(nt,numel(PD_new_tmp.ancillary.delta_energy_plus));
          delta_energy_minus = zeros(nt,numel(PD_new_tmp.ancillary.delta_energy_minus));

        else
          %PD = PD.combine(PD_new_tmp);
        end

        % Fill in values
        % Main
        data(it,:,:,:)           = PD_new_tmp.data;
        energy(it,:,:,:)         = PD_new_tmp.depend{1};
        azim(it,:,:,:)           = PD_new_tmp.depend{2};
        polar(it,:,:,:)          = PD_new_tmp.depend{3};

        % Ancillary
        V_edges(it,:)            = PD_new_tmp.ancillary.V_edges;
        phi_edges(it,:)          = PD_new_tmp.ancillary.phi_edges;
        theta_edges(it,:)        = PD_new_tmp.ancillary.theta_edges;
        energy0(it,:)            = PD_new_tmp.ancillary.energy0;
        energy1(it,:)            = PD_new_tmp.ancillary.energy1;
        esteptable(it,:)         = PD_new_tmp.ancillary.esteptable;
        energy(it,:)             = PD_new_tmp.ancillary.energy;
        delta_energy_plus(it,:)  = PD_new_tmp.ancillary.delta_energy_plus;
        delta_energy_minus(it,:) = PD_new_tmp.ancillary.delta_energy_plus;
      end

      % Construct PDist
      PD = PDist(pdist.time,data,'skymap',energy,azim,polar(1,:)); % some functions cannot take the polar angle as a time series 

      PD.ancillary.V_edges = V_edges/1000;%km/s
      PD.ancillary.phi_edges = phi_edges;
      PD.ancillary.theta_edges = theta_edges;
      PD.ancillary.base = 'sph';
      PD.units = pdist.units;
      PD.species = pdist.species;
      ...
      while have_options
        ...
        switch(lower(args{1}))
          ...
          case 'scpot'
            ...
            elseif isnumeric(scpot) && isscalar(scpot)
    ...
    function PD = azimuthangle(obj)

      PD_old = obj;
      data_old = PD_old.data;
      data_new = squeeze(nanmean(data_old,4)); % mean over polar angles
      PD = PDist(PD_old.time,data_new,'azimuthangle',PD_old.depend{1},PD_old.depend{2});
    end
