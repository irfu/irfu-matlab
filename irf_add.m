...
if size(x1,2)>size(x2,2),
  disp('irf_add(c1,x1,c2,x2) WARNING: x1 has more columns than x2');
  ...
  else, disp('irf_add() ERROR: could not make inteligent guess what you are meaning.');return; 
  end
elseif size(x2,2)>size(x1,2),
  disp('irf_add(c1,x1,c2,x2) WARNING: x2 has more columns than x1');
...
if size(x1,1) ~= size(x2,1), 
 if debug == 1,disp('interpolating x2 to x1. irf_add(c1,x1,c2,x2)');end
 x2=av_interp(x2,x1);
end
