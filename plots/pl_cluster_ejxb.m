...
% function [Blnm]=p1052_ar(tint,Vmp,L,Nsign,flagEdotB);
%
...
%
% Usage: [blnm]=p1052(toepoch([2002 03 30 13 11 42])+[0 8],31.4* [ -0.94 -0.21 -0.25],[0 0 1],-1,0);
...
c_eval('dtn?=NVps?;dtn?(1,2)=0;dtn?(2:end,1)=0.5*(dtn?(2:end,1)+dtn?(1:end-1,1));');
c_eval('dtn?(2:end,2)=(NVps?(2:end,2)-NVps?(1:end-1,2))./(NVps?(2:end,1)-NVps?(1:end-1,1));');
c_eval('vn?=irf_resamp([0 v],dtn?);');
c_eval('egradn?=[dtn?(:,1) Te./irf_abs(vn?,1)./NVps?(:,2).*dtn?(:,2)];');
c_eval('egradn3d?=irf_vec_x_scal(irf_tappl(irf_norm(vn?),''*(-1)''),egradn?);');

sc_list=irf_ask('For which s/c plot single s/c plots ?[%]>','sc_list',[]);

icb=irf_ask('For which s/c to include internal burst ?[%]>','ic',[]);
for j=1:length(icb),
  sc_id=ic(j);
  c_load('P4kHz?p4',sc_id);
  c_eval('nburst?=c_efw_scp2ne(P4kHz?p4);nburst?(1:2,:)=[];nburst?=irf_filt(nburst?,0,180);',sc_id);
  ...
  c_eval('egradn_burst?=[dtn?(:,1) Te./irf_abs(vn?,1)./nburst?(:,2).*dtn?(:,2)];',sc_id);
...
axes(h(1));
c_pl_tx(Blnm1,Blnm2,Blnm3,Blnm4,2,1,dt.*0);ylabel('B_L [nT]');
...
axes(h(2));
c_pl_tx(Blnm1,Blnm2,Blnm3,Blnm4,2,1,dt);ylabel('B_L [nT]');

axes(h(3));
c_pl_tx(Blnm1,Blnm2,Blnm3,Blnm4,4,1,dt);ylabel('B_M [nT]');

axes(h(4));
c_pl_tx(Blnm1,Blnm2,Blnm3,Blnm4,3,1,dt);ylabel('B_N [nT]');
...
axes(h(6));
c_pl_tx(dEm2(1,:),dEm2,dEm3,dEm4,2,1,dt);ylabel('E_{tang} [mV/m]');
%  c_pl_tx(dEm2(1,:),dEnfw2,irf_tappl(dEnfw3,'+50'),irf_tappl(dEnfw4,'+100'),2,1,dt);
ylabel('E_{M} [mV/m]');
ht=irf_pl_info(['f=20-180Hz'],gca,[0.05,.8]); set(ht,'interpreter','none','FontSize', 10);

axes(h(7));
c_pl_tx(NVps1,NVps2,NVps3,NVps4,2,1,dt);
ylabel('N_{Vps} [cc]');
...
for ic=sc_list;
  ...
  axes(h(4));
  c_eval('irf_plot(irf_tappl([jz? jperp?(:,2)],''*1e6''));ylabel(''j_\|,jperp [\mu A/m2] sc?'');',ic);
