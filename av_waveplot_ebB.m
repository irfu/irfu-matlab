function h=av_waveplot_ebB(e,b,B,parameters)

%AV_WAVEPLOT_ebB(e,b,B,parameters) plot of your choice
...
elseif nargin == 3,
  ...
  if     sampl_b > 1.5*sampl_e, e=av_interp(e,b); sampl=sampl_b; disp('av_waveplot_ebB: interpolating e to b');
  elseif sampl_e > 1.5*sampl_b, b=av_interp(b,e); sampl=sampl_e; disp('av_waveplot_ebB: interpolating b to e');
  else   sampl=2*sampl_e; t=max(e(1,1),b(1,1)):1/sampl:min(e(end,1),b(end,1)); t=t'; e=av_interp(e,t); b=av_interp(b,t); disp('at_waveplot_ebB: interpolating b and e to 2x e sampling');
  end
...
disp('av_waveplot_ebB ... calculate e and b wavelet transform ....');
...
%  [aa,Sww]=meshgrid(a,Sw); % Matrix form
  [aa,Swwex]=meshgrid(a,Swex);[aa,Swwey]=meshgrid(a,Swey);[aa,Swwez]=meshgrid(a,Swez);
  [aa,Swwbx]=meshgrid(a,Swbx);[aa,Swwby]=meshgrid(a,Swby);[aa,Swwbz]=meshgrid(a,Swbz);
%% Calculate the FFT of the wavelet transform
  Wwex=sqrt(1).*Swwex.*exp(-sigma*sigma*((aa.*ww-w0).^2)/2);
  Wwey=sqrt(1).*Swwey.*exp(-sigma*sigma*((aa.*ww-w0).^2)/2);
  Wwez=sqrt(1).*Swwez.*exp(-sigma*sigma*((aa.*ww-w0).^2)/2);
  Wwbx=sqrt(1).*Swwbx.*exp(-sigma*sigma*((aa.*ww-w0).^2)/2);
  Wwby=sqrt(1).*Swwby.*exp(-sigma*sigma*((aa.*ww-w0).^2)/2);
  Wwbz=sqrt(1).*Swwbz.*exp(-sigma*sigma*((aa.*ww-w0).^2)/2);
...
disp('av_waveplot_ebB ... calculate all spectra ....');
  ...
  [aaa,bnx]=meshgrid(a,bn(:,2));
  [aaa,bny]=meshgrid(a,bn(:,3));
  [aaa,bnz]=meshgrid(a,bn(:,4));

  Spar=Sx.*bnx+Sy.*bny+Sz.*bnz;
...
if q_spectra_width,
  if q_spectra_width<length(t),
    nav=ceil(length(t)/q_spectra_width);
    tt=t(1:nav:length(t));clear t;t=tt;
    for jj=1:length(t)-1
      power2E_plot(jj,:)=sum(power2E((jj-1)*nav+1:jj*nav,:),1)/nav;
      power2B_plot(jj,:)=sum(power2B((jj-1)*nav+1:jj*nav,:),1)/nav;
      Spar_plot(jj,:)=sum(Spar((jj-1)*nav+1:jj*nav,:),1)/nav;
    end
      last_point=length(t);n_points=size(power2E,1);
      last_interval=(last_point-1)*nav+1:max(jj*nav,n_points);
      n_in_last_interval=length(last_interval);
      power2E_plot(last_point,:)=sum(power2E(last_interval,:),1)/n_in_last_interval;
      power2B_plot(last_point,:)=sum(power2B(last_interval,:),1)/n_in_last_interval;
      Spar_plot(last_point,:)   =sum(   Spar(last_interval,:),1)/n_in_last_interval;
  end
else,
  power2E_plot=power2E;
  power2B_plot=power2B;
  Spar_plot=Spar;
end
