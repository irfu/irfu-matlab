function out = EncodeEpoch(epoch, varargin)
%EncodeEpoch encodes an epoch of CDF_EPOCH data type, a double value or
%cdfepoch object..
%
%   OUT = EncodeEpoch(epoch) 
...
%
%   OUT = ENCODEEPOCH(epoch, 'Format', FORMAT) encodes the UTC
...
%
%   Note: If the epoch values come from cdfread function call, the values
%         can be in one of the three forms: in cdfepoch object, in MATLAB
%         datenum via 'combinerecords' option, or in their original CDF_EPOCH 
...
%
%   epochs = cdfread('Sample', 'Variables', {'Epoch'});
%   encodeepoch(epochs)
%
%   % Or,
%
%   epochs = cdfread('Sample', 'Variables', {'Epoch'}, 'combinerecords', ...
%                    true, 'keepepochasis', true);
%   encodeepoch(epochs)
%
%   % Or,
%
...
%   datestr(epochs)
%
%   See also CDFEPOCH, ENCODETT2000.
...
if (nargin < 1)
    error('MATLAB:ENCODEEPOCH:inputArgumentCount', ...
          'ENCODEEPOCH requires at least one input argument.')
end

if (nargout > 1)
    error('MATLAB:ENCODEEPOCH:outputArguments', ...
          'ENCODEEPOCH requires only one output argument.')
end

[args, msg] = parse_inputs(varargin{:});
if (~isempty(msg))
    error('MATLAB:ENCODEEPOCH:badInputArguments', '%s', msg)
end
if (isa(epoch,'cdfepoch'))
  ...
  out = encodeepochc(epoch, args.Format);
end
%%%
