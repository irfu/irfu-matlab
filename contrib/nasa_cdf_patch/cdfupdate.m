function cdfupdate(filename, varargin)
%CDFUPDATE Update data in an existing CDF file.
% 
%   CDFUPDATE(FILE, 'VariableData', VARIABLELIST, ...) updates variable data 
...
%
%
%   CDFUPDATE(..., 'GlobalAttributes', GATTRIB, ...) updates the structure
...
%   cell array.
%
%   CDFUPDATE(..., 'VariableAttributes', VATTRIB, ...) updates the
...
%
%   CDFUPDATE(..., 'CDFCompression', TF, ...) resets the CDF compression.
...
%
%      cdfupdate('example', 'VariableData', {'Longitude' [{0,{3},100};{1,{2},200}] ...
...
%      cdfupdate('example', 'VariableData', [var1 var2 var3]);
%
...
%   cdfupdate('example', 'GlobalAttributes', globalAttribStruct);
%
...
if (nargin < 2)
    error('MATLAB:cdfupdate:inputArgumentCount', ...
...
%
% Call the underlying cdfupdatec function which calls the CDFlib
%
...
if (nargin > 0)
    ...
    for k = 1:2:length(varargin)
        ...
        if (~ischar(param))
            ...
            exception.id = 'MATLAB:cdfupdate:paramNameNotString';
            return
        end
        ...
        if (isempty(idx))
            ...
            exception.id = 'MATLAB:cdfupdate:unrecognizedParam';
            return
        elseif (length(idx) > 1)
            exception.msg = sprintf('Ambiguous parameter name "%s".', param);
             exception.id = 'MATLAB:cdfupdate:ambiguousParam';
           return
        end
        ...
        case 'globalattributes'
           ...
           if ~isstruct(globalAttribStruct)
               ...
               exception.id = 'MATLAB:cdfupdate:globalAttributesNotStruct';
               return
           end
        ...
        case 'variabledata'
           ...
           if ~iscell(varcell)
               ...
               exception.id = 'MATLAB:cdfupdate:variabledatanotcell';
               return
           end
           ...
           if length(args.VarNames) ~= length(args.VarVals)
               ...
               exception.id = 'MATLAB:cdfupdate:variableWithoutValue';
               return
           end
           % args.VarVals
        ...
        case 'variableattributes'
           ...
           if ~isstruct(varAttribStruct)
               ...
               exception.id = 'MATLAB:cdfupdate:variableAttributesNotStruct';
               return
           end
