...
%       end
    ...
    function PD = e64(obj)
      ...
      if ~strcmp(obj.type_,'skymap'); error('PDist must be a skymap.'); end 
      %if size(unique(obj.depend{1},'rows'),1) == 1; error('PDist only has a single energy level. PDist.e64 not applicable.'); end 
      if size(unique(obj.depend{1},'rows'),1) == 1; PD = obj; irf.log('warning','PDist only has a single energy level. Returning original input PDist.'); return; end 
      if size(obj.depend{1},2) == 64; irf.log('warning','PDist already has 64 energy levels.'); end 
      ...
      % delta energy plus / minus, from mms.psd_moments
      energy_plus = 10.^(log10(PD.depend{1}(:,1:63))+diff(PD.depend{1}(:,2:64),2)/2);
      energy_minus = 10.^(log10(PD.depend{1}(:,2:64))-diff(PD.depend{1}(:,1:63))/2);
      energyall = PD.depend{1};
      ...
      energy_plus = 10.^(log10(energyall)+diffenall(2:64)/2);
      energy_minus = 10.^(log10(energyall)-diffenall(1:63)/2);
      ...
      if isfield(PD.ancillary,'energy'), PD.ancillary.energy = PD.depend{1}; end
      if isfield(PD.ancillary,'delta_energy_minus'), PD.ancillary.delta_energy_minus = energy_minus; end
      if isfield(PD.ancillary,'delta_energy_plus'), PD.ancillary.delta_energy_plus = energy_plus; end
