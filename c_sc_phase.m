function phase_out = c_sc_phase(t,phase_2)
% function phase_out = c_sc_phase(t,phase_2)
% function phase_out = c_sc_phase(t,ic)
%
% Find spacecraft phase for give time vector t
%
% t - column vector with time in isdat epoch
% phase_2 - column vector [time phase_2]
%   if phase_2 is one number assume that it is sc_number and download phase from isdat 'disco:10'
...
if prod(size(phase_2))==1, % phase_2 is sc number, download phase_2 from isdat 
  ...
  [t_isdat,ph2]=ISGet('disco:10',fromepoch(tint(1)),tint(2)-tint(1),ic,'ephemeris', 'phase_2');
  phase_2=[t_isdat ph2];
end  % 
